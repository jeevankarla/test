...
//Debug.logInfo("[Processing]: SOAP parameters="+parameters, module);

            String tenantId = (String)parameters.get("tenantId");
			if (UtilValidate.isNotEmpty(tenantId)) {
				String tenantDelegatorName = delegator.getDelegatorBaseName() + "#" + tenantId;
				request.getSession().setAttribute("delegatorName", tenantDelegatorName);
				// after this line the delegator is replaced with the new per-tenant delegator
				Delegator newDelegator = DelegatorFactory.getDelegator(tenantDelegatorName);
				request.getSession().getServletContext().setAttribute("delegator", newDelegator);
                dispatcher = ContextFilter.makeWebappDispatcher(request.getSession().getServletContext(), newDelegator);
