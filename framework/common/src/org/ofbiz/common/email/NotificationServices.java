...
public class NotificationServices {
    ...
    public static void setBaseUrl(Delegator delegator, String webSiteId, Map<String, Object> context) {
        ...
        if (!context.containsKey("baseUrl")) {
            ...
            // fill in any missing properties with fields from tenant configuration or finally the global file
            try {
            	GenericValue forceHttpHost = delegator.findByPrimaryKeyCache("TenantConfiguration",
        				UtilMisc.toMap("propertyTypeEnumId", "GLOBAL", "propertyName", "force.http.host"));
                if (forceHttpHost != null) {
                	httpServer = forceHttpHost.getString("propertyValue");
                } 
            	GenericValue portHttp = delegator.findByPrimaryKeyCache("TenantConfiguration",
        				UtilMisc.toMap("propertyTypeEnumId", "GLOBAL", "propertyName", "port.http"));
                if (portHttp != null) {
                	httpPort = portHttp.getString("propertyValue");
                }                  
            	GenericValue forceHttpsHost = delegator.findByPrimaryKeyCache("TenantConfiguration",
    				UtilMisc.toMap("propertyTypeEnumId", "GLOBAL", "propertyName", "force.https.host"));
            	if (forceHttpsHost != null) {
            		httpsServer = forceHttpsHost.getString("propertyValue");
            	} 
            	GenericValue portHttps = delegator.findByPrimaryKeyCache("TenantConfiguration",
        				UtilMisc.toMap("propertyTypeEnumId", "GLOBAL", "propertyName", "port.https"));
                if (portHttps != null) {
                	httpsPort = portHttps.getString("propertyValue");
                }              	
            } catch (Exception e) {
            	Debug.logWarning("Unable to retrieve TenantConfiguration:" + e, module);
            }
