...
customerAddressDetails = delegator.findList("PartyContactDetailByPurpose", conditionAddress, null, null, null, false);
customerAdd=[:];

if(customerAddressDetails){
	customerAddress=customerAddressDetails.getFirst();
	if(customerAddress){
		//partyPostalAddress=contactMec.get("postalAddress");
		
		partyPostalAddress=customerAddress;
	...
	//	partyPostalAddress= dispatcher.runSync("getPartyPostalAddress", [partyId:invoicePartyId, userLogin: userLogin]);
		if(partyPostalAddress){
			...
			customerAdd.put("toName",customerName);
			customerAdd.put("address1",address1);
			customerAdd.put("address2",address2);
			customerAdd.put("city",city);
			customerAdd.put("postalCode",postalCode);
