    ...
    <div class="screenlet-title-bar">
      <ul>
        ...
        <#if security.hasEntityPermission("ORDERMGR", "_NOTE", session)>
          <li><a href="<@ofbizUrl>creatNewnote?${paramString}</@ofbizUrl>">${uiLabelMap.OrderNotesCreateNew}</a></li>
        </#if>
      </ul>
      <br class="clear"/>
    </div>
                ...
                <tr>
                  ...
                  <td align="right" valign="top" width="15%">
                    <#if note.internalNote?if_exists == "N">
                        ...
                        <form name="privateNotesForm_${note_index}" method="post" action="<@ofbizUrl>updatePONote</@ofbizUrl>">
                    ...
                    <#if note.internalNote?if_exists == "Y">
                        ...
                        <form name="publicNotesForm_${note_index}" method="post" action="<@ofbizUrl>updatePONote</@ofbizUrl>">
                          <input type="hidden" name="orderId" value="${orderId}"/>
                          <input type="hidden" name="noteId" value="${note.noteId}"/>
