...
<script type="application/javascript">
	...
	function showPoReportEmail(supplierPartyId,orderId) {
	    showSpinner();
		globalSupplierPartyId=supplierPartyId;
       var dataString="orderId=" + orderId + "&supplierPartyId=" + supplierPartyId ;
      $.ajax({
             type: "POST",
		...
		var message = "";
		message += "<div style='width:100%;height:350px;overflow-x:auto;overflow-y:auto;' ><form action='sendPoReportEmailTosupplier' method='post' onsubmit='return disableGenerateButton();'><table cellspacing=10 cellpadding=10  width='100%' > " ; 
			...
			"</td></tr>";
		   message += "<tr ><td align='right' class='h2' width='15%' >Subject &nbsp;: </td><td align='left' class='h2' width='75%'  >&nbsp;<input type='text' size='63'  id='subject' name='subject'/><input type='hidden' id='partyId'  name='partyId' value='"+supplierPartyId+"'  /><input type='hidden' id='orderId'  name='orderId' value='"+orderId+"'  /> </tr></tbody></table></td></tr>";
			 message += "<tr ><td width='100%' colspan='2' ><table  border='0' cellspacing='10' cellpadding='10'><tbody><tr><td width='15%' align='left' class='label labelFontCSS' >Content:</td><td align='left'  width='75%'  >";
              message += "<textarea name='bodyText' id='bodyText' cols='70' rows='10'></textarea> </td></tr>";
		...
		message += "</div>";				
		var title = "<center  class='h2' >Send Po Report Email <center> ";
		Alert(message, title);
	};
