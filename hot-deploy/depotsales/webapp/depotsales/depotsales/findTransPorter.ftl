...
<#if partyList?has_content>
    <ul>
  <#if hideFields == "Y">
      <li class="collapsed"><a href="<@ofbizUrl>findTransporter?hideFields=N${paramList}</@ofbizUrl>" title="${uiLabelMap.CommonShowLookupFields}">&nbsp;</a></li>
  <#else>
      <li class="expanded"><a href="<@ofbizUrl>findTransporter?hideFields=Y${paramList}</@ofbizUrl>" title="${uiLabelMap.CommonHideFields}">&nbsp;</a></li>
  </#if>
  <#if (partyListSize > 0)>
    <#if (partyListSize > highIndex)>
      <li><a class="nav-next" href="<@ofbizUrl>findTransporter?VIEW_SIZE=${viewSize}&amp;VIEW_INDEX=${viewIndex+1}&amp;hideFields=${hideFields}${paramList}</@ofbizUrl>">${uiLabelMap.CommonNext}</a></li>
    <#else>
      <li class="disabled">${uiLabelMap.CommonNext}</li>
    </#if>
      <li>${lowIndex} - ${highIndex} ${uiLabelMap.CommonOf} ${partyListSize}</li>
    <#if (viewIndex > 0)>
      <li><a class="nav-previous" href="<@ofbizUrl>findTransporter?VIEW_SIZE=${viewSize}&amp;VIEW_INDEX=${viewIndex-1}&amp;hideFields=${hideFields}${paramList}</@ofbizUrl>">${uiLabelMap.CommonPrevious}</a></li>
    <#else>
  ...
  <div class="screenlet-body">
    <div id="findPartyParameters" <#if hideFields != "N"> style="display:none" </#if> >
      ...
      <#-- NOTE: this form is setup to allow a search by partial partyId or userLoginId; to change it to go directly to
          the TransPorterviewprofile page when these are entered add the follow attribute to the form element:

           onsubmit="javascript:lookupParty('<@ofbizUrl>TransPorterviewprofile</@ofbizUrl>');"
       -->
      <form method="post" name="lookupparty" action="<@ofbizUrl>findTransporter</@ofbizUrl>" class="basic-form">
          ...
          <tr>
           ...
           <tr>
               <td class="label">${uiLabelMap.RoleType}</td>
	            <td>
	             <#assign roleType = delegator.findByAnd("RoleType", {"roleTypeId" : "TRANSPORT_CONTRACTOR"})/>
  ...
  <#if partyList?has_content>
    ...
    <#list partyList as partyRow>
      ...
      <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
        <td><a href="<@ofbizUrl>TransPorterviewprofile?partyId=${partyRow.partyId}</@ofbizUrl>">${partyRow.partyId}</a></td>
        <td>
      ...
      <#assign mainRole = dispatcher.runSync("getPartyMainRole", Static["org.ofbiz.base.util.UtilMisc"].toMap("partyId", partyRow.partyId, "userLogin", userLogin))/>
            ...
            <td class="button-col align-float">
              <a href="<@ofbizUrl>TransPorterviewprofile?partyId=${partyRow.partyId}</@ofbizUrl>">${uiLabelMap.CommonDetails}</a>
