	...
	function showPaymentEntryQTip(partyIdFrom1,partyIdTo1,invoiceId1,voucherType1,amount1,partyNameTemp,purposeTypeId1) {
		...
		function showCancelInvoiceQtip(invoiceId){
			var invoiceId = invoiceId;
	     	var message = "";
	            message += "<html><head></head><body><form id='invoicestatuschange' method='post' action='cancelPurchaseInvoiceDC' onsubmit='return disableGenerateButton();'><table cellspacing=10 cellpadding=10 width=400>";
	        ...
	        <#list invoiceList as invoice>
	          ...
	          <#assign invoicePaymentInfo = invoicePaymentInfoList.get("invoicePaymentInfoList").get(0)?if_exists>
	            ...
	            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
	              <td><a class="buttontext" target='_blank' href="<@ofbizUrl>APInvoiceOverviewDC?invoiceId=${invoice.invoiceId}&amp;subTabButtonValue=${tabButtonItem5}</@ofbizUrl>">${invoiceSequence}</a></td>
	              <#--><td>
