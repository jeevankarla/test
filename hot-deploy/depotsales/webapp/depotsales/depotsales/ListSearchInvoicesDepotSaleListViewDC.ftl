...
function showPaymentEntryQTip(partyIdFrom1,partyIdTo1,invoiceId1,voucherType1,amount1,partyNameTemp,purposeTypeId1) {
	...
	function showCancelInvoiceQtip(invoiceId){
		var invoiceId = invoiceId;
     	var message = "";
            message += "<html><head></head><body><form id='invoicestatuschange' method='post' action='cancelDepotSalesInvoiceDC' onsubmit='return disableGenerateButton();'><table cellspacing=10 cellpadding=10 width=400>";
    ...
    <table class="basic-table hover-bar" cellspacing="0">
      ...
      <tbody>
        ...
        <#list invoiceList as invoice>
          ...
          <#assign invoiceSequence = Static["org.ofbiz.accounting.invoice.InvoiceServices"].getInvoiceSequence(delegator, invoice.invoiceId)?if_exists/>
            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
              <td><a class="buttontext" href="<@ofbizUrl>DepotTransSalesInvoiceOverviewDC?invoiceId=${invoice.invoiceId}&amp;subTabButtonValue=${tabButtonItem5}</@ofbizUrl>">${invoiceSequence}</a></td>
              <td>
        ...
        <#-->       <#if ((invoice.statusId != "INVOICE_IN_PROCESS") && (invoice.statusId != "INVOICE_CANCELLED") && (invoicePaymentInfo.outstandingAmount >0)) >
                ...
                <#else>
                   ...
                   <#if ( ((invoice.statusId == "INVOICE_IN_PROCESS") || (invoice.statusId == "INVOICE_READY")) && (invoice.statusId != "INVOICE_CANCELLED") && (invoicePaymentInfo.outstandingAmount >0)) >
              	  <#if (invoice.invoiceTypeId == "SALES_INVOICE")||(invoice.invoiceTypeId?exists) >
              		   ...
              		   <td><input type="button" name="Payment" id="Payment" value="Payment" onclick="javascript:showPaymentEntryForDepotInvoListing('${invoice.invoiceId}','${invoicePaymentInfo.amount}','${invoicePaymentInfo.outstandingAmount}','${invoice.partyId}','${invoice.partyIdFrom}','${vendorName}','${purposeTypeId}');"/></td>
               	    <#else>
                	  <td align="center"></td>
               	  </#if>
                <#else>
              ...
              <#if invoice.statusId != "INVOICE_CANCELLED"> <td><a class="buttontext" target='_blank' href="<@ofbizUrl>MaterialDepotSalesEditInvoiceInitDC?invoiceId=${invoice.invoiceId}&amp;partyId=${invoice.partyId}&amp;partyName=${Static["org.ofbiz.party.party.PartyHelper"].getPartyName(delegator, invoice.partyId, false)}</@ofbizUrl>">Edit Invoice</a></td>
                <#else>
 		   	       <td></td>
              </#if>
              
              <#if invoice.statusId != "INVOICE_CANCELLED"><td><a class="buttontext" target="_BLANK" href="<@ofbizUrl>newInvoiceDEPOTVoucherDC?invoiceId=${invoice.invoiceId}</@ofbizUrl>">Print</a></td> </#if>
    ...
    <table class="basic-table hover-bar" cellspacing="0">
      ...
      <tbody>
        ...
        <#-->       <#if ((invoice.statusId != "INVOICE_IN_PROCESS") && (invoice.statusId != "INVOICE_CANCELLED") && (invoicePaymentInfo.outstandingAmount >0)) >
              ...
              <#if invoice.statusId != "INVOICE_CANCELLED"><td><a class="buttontext" target="_BLANK" href="<@ofbizUrl>newInvoiceDEPOTVoucher?invoiceId=${invoice.invoiceId}</@ofbizUrl>">Print</a></td> </#if>
              <td align="center"><#if invoice.statusId != "INVOICE_CANCELLED"><input type="button" name="cancel" value="Cancel" onclick="javascript:confirmInvoiceCancel('${invoice.invoiceId}')"/></#if></td>
