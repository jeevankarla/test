...
<form name="paymentSubmitForm" id="paymentSubmitForm" method="post" action="makeMassInvoicePaymentsForDepot">
</form>
  <div>
          ...
          <#assign invoicePaymentInfo = invoicePaymentInfoList.get("invoicePaymentInfoList").get(0)?if_exists>
            ...
            <#--  <td>${(invoice.dueDate)?if_exists}</td> -->
             ...
             <#--- <td>${(invoice.description)?if_exists}</td>
              ...
              <input type = "hidden" name = "partyId" id = "partyId" value = "${invoice.costCenterId}">
              ...
              <td><a href="/partymgr/control/viewprofile?partyId=${invoice.costCenterId}">${Static["org.ofbiz.party.party.PartyHelper"].getPartyName(delegator, invoice.costCenterId, false)?if_exists} [${(invoice.costCenterId)?if_exists}]</a></td>
              ...
              <#--- <td><a class="buttontext" target="_BLANK" href="<@ofbizUrl>purchaseInvoiceDetails.pdf?invoiceId=${invoice.invoiceId}</@ofbizUrl>">Print</a></td>-->
               ...
               <#if invoice.statusId != "INVOICE_CANCELLED"><td><a class="buttontext" target='_blank' href="<@ofbizUrl>DepotpurchaseInvoiceEditDC?invoiceId=${invoice.invoiceId}&amp;partyId=${invoice.partyId}&amp;partyName=${Static["org.ofbiz.party.party.PartyHelper"].getPartyName(delegator, invoice.partyId, false)}</@ofbizUrl>">Edit Invoice</a></td>
				...
				<td>
					<a class="buttontext" target='_blank' href="<@ofbizUrl>DepotpurchaseInvoiceReportDC.pdf?invoiceId=${invoice.invoiceId}</@ofbizUrl>">Print</a>
				</td>
