...
<script type="application/javascript">
	...
	function raiseSalesInvoiceFromBranch(shipmentId,partyIdTo,partyName,orderId,billToPartyId,purchaseInvoiceId) {	
		...
		title = "<center>Sales Invoice<center><br />";
		message += "</table></form>";
		Alert(message, title);
	};
	...
	function LoomEditForm(partyId,loomTypeId,quantity,fromDate,thruDate) {	
	
		var oldFromDate = fromDate; 
		var message = "";
		message += "<form action='UpdatePartyLoom' method='post' onsubmit='return disableSubmitButton();'><table cellspacing=10 cellpadding=10>";
		
		message +=  "<tr class='h3'><td align='left' class='h3' width='50%'>Party Id:</td><td align='left' width='50%'><input class='h3' type='text' readonly id='partyId' name='partyId' value='"+partyId+"'/></td></tr>";
