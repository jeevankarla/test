...
if(reportType=="ADV_RECEIVED"){
	partyIds = EntityUtil.getFieldListFromEntityList(advancePaymentList, "partyIdFrom", true);
}
else{
...
for(eachPartyId in partyIds){
	...
	if(reportType=="ADV_RECEIVED"){
		advancePaymentForEachId = EntityUtil.filterByCondition(advancePaymentList, EntityCondition.makeCondition("partyIdFrom", EntityOperator.EQUALS, eachPartyId));
	}
	else{
		advancePaymentForEachId = EntityUtil.filterByCondition(advancePaymentList, EntityCondition.makeCondition("partyIdTo", EntityOperator.EQUALS, eachPartyId));
	}
	paymentIds = EntityUtil.getFieldListFromEntityList(advancePaymentForEachId, "paymentId", true);
	...
	for(eachPaymentId in paymentIds){
		...
		advancePaymentForPaymentId = EntityUtil.filterByCondition(advancePaymentForEachId, EntityCondition.makeCondition("paymentId", EntityOperator.EQUALS, eachPaymentId));
		
		firstMntAdvPmnt = EntityUtil.filterByCondition(advancePaymentForPaymentId, EntityCondition.makeCondition(EntityCondition.makeCondition("paymentDate",EntityOperator.GREATER_THAN_EQUAL_TO, fStMnthfromDate) ,EntityOperator.AND,
			EntityCondition.makeCondition("paymentDate",EntityOperator.LESS_THAN_EQUAL_TO,fStMnththruDate)));
		
		secndMntAdvPmnt = EntityUtil.filterByCondition(advancePaymentForPaymentId, EntityCondition.makeCondition(EntityCondition.makeCondition("paymentDate",EntityOperator.GREATER_THAN_EQUAL_TO, sndMnthfromDate) ,EntityOperator.AND,
			EntityCondition.makeCondition("paymentDate",EntityOperator.LESS_THAN_EQUAL_TO,sndMnththruDate)));
		
		if(days == "30days"){
			thrdMntAdvPmnt = EntityUtil.filterByCondition(advancePaymentForPaymentId, EntityCondition.makeCondition(EntityCondition.makeCondition("paymentDate",EntityOperator.GREATER_THAN_EQUAL_TO, thrdMnthfromDate) ,EntityOperator.AND,
				EntityCondition.makeCondition("paymentDate",EntityOperator.LESS_THAN_EQUAL_TO,thrdMnththruDate)));
		}
		frthToSixthMntAdvPmnt = EntityUtil.filterByCondition(advancePaymentForPaymentId, EntityCondition.makeCondition(EntityCondition.makeCondition("paymentDate",EntityOperator.GREATER_THAN_EQUAL_TO, frthMnthfromDate) ,EntityOperator.AND,
			EntityCondition.makeCondition("paymentDate",EntityOperator.LESS_THAN_EQUAL_TO,frthMnththruDate)));
		
		sixthToOneYrAdvPmnt = EntityUtil.filterByCondition(advancePaymentForPaymentId, EntityCondition.makeCondition(EntityCondition.makeCondition("paymentDate",EntityOperator.GREATER_THAN_EQUAL_TO, fromfifthMnthfromDate) ,EntityOperator.AND,
			EntityCondition.makeCondition("paymentDate",EntityOperator.LESS_THAN_EQUAL_TO,fromfifthMnththruDate)));
		
		oneToTwoYrsAdvPmnt = EntityUtil.filterByCondition(advancePaymentForPaymentId, EntityCondition.makeCondition(EntityCondition.makeCondition("paymentDate",EntityOperator.GREATER_THAN_EQUAL_TO, firstyearfromDate) ,EntityOperator.AND,
			EntityCondition.makeCondition("paymentDate",EntityOperator.LESS_THAN_EQUAL_TO,firstyearthruDate)));
		
		secndToThirdYrAdvPmnt = EntityUtil.filterByCondition(advancePaymentForPaymentId, EntityCondition.makeCondition(EntityCondition.makeCondition("paymentDate",EntityOperator.GREATER_THAN_EQUAL_TO, secondyearfromDate) ,EntityOperator.AND,
			EntityCondition.makeCondition("paymentDate",EntityOperator.LESS_THAN_EQUAL_TO,secondyearthruDate )));
		above3YrsAdvPmnt = EntityUtil.filterByCondition(advancePaymentForPaymentId, EntityCondition.makeCondition(EntityCondition.makeCondition("paymentDate",EntityOperator.LESS_THAN_EQUAL_TO,secondyearfromDate)));
