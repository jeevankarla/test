...
//inventoryItemFind
	...
	physicalInventory.each { iter ->
		...
		//=================== specification====================================
		invCondList = [];
		invCondList.add(EntityCondition.makeCondition("inventoryItemId", EntityOperator.EQUALS , iter.inventoryItemId));
		invCondList.add(EntityCondition.makeCondition("orderId", EntityOperator.NOT_EQUAL, null));
		invCondList.add(EntityCondition.makeCondition("orderItemSeqId", EntityOperator.NOT_EQUAL, null));
		tempInvDetail = delegator.findList("InventoryItemDetail",  EntityCondition.makeCondition(invCondList, EntityOperator.AND),null, null, null, false );
		String specification = "";
		
		if(UtilValidate.isNotEmpty(tempInvDetail)){
			tempInvDetail = EntityUtil.getFirst(tempInvDetail);
			/*invCondList.clear();
			invCondList.add(EntityCondition.makeCondition("orderId", EntityOperator.EQUALS, tempInvDetail.get("orderId")));
			invCondList.add(EntityCondition.makeCondition("orderItemSeqId", EntityOperator.EQUALS, tempInvDetail.get("orderItemSeqId")));
			invCondList.add(EntityCondition.makeCondition("attrName", EntityOperator.EQUALS, "REMARKS"));*/
			tempOrderDetail = delegator.findOne("OrderItemAttribute", UtilMisc.toMap("orderId", tempInvDetail.get("orderId"),"orderItemSeqId",tempInvDetail.get("orderItemSeqId"),"attrName","REMARKS"), false);
			if(UtilValidate.isNotEmpty(tempOrderDetail)){
				specification = tempOrderDetail.get("attrValue");
			}
		}
		row.putAt("specification", specification);
