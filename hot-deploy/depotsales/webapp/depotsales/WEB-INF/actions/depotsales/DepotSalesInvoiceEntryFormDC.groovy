...
if(shipments){
	...
	//}
		...
		orderItems = delegator.findList("OrderItem", EntityCondition.makeCondition("orderId", EntityOperator.EQUALS, actualOrderId), null, null, null, false);
		
		orderAdjustmentsList = delegator.findList("OrderAdjustment", EntityCondition.makeCondition("orderId", EntityOperator.EQUALS, actualOrderId), null, null, null, false);
		...
		for (eachItem in orderItems) {
			...
			for(i=0; i<taxList.size(); i++){
				...
				taxItem = (taxList.get(i)).get("orderAdjustmentTypeId");
				
				taxItem = taxItem.replace("SALE", "PUR");
				
				
				if(taxItem != "VAT_SURCHARGE" && taxItem != "CST_SURCHARGE" ){
				newObj.put(taxItem, (taxList.get(i)).get("sourcePercentage"));
				newObj.put(taxItem + "_AMT", (taxList.get(i)).get("amount"));
				}else{
				newObj.put(taxItem+"_PUR", (taxList.get(i)).get("sourcePercentage"));
				newObj.put(taxItem + "_PUR_AMT", (taxList.get(i)).get("amount"));
				}
			...
			cond.clear();
			cond.add(EntityCondition.makeCondition("orderItemSeqId", EntityOperator.EQUALS,eachItem.orderItemSeqId));
			cond.add(EntityCondition.makeCondition("orderAdjustmentTypeId", EntityOperator.NOT_IN, ["VAT_SALE","VAT_SURCHARGE","CST_SALE","CST_SURCHARGE","TEN_PERCENT_SUBSIDY","SERVICE_CHARGE"]));
			...
			newObj.put("itemAdjustments", itemAdjustmentJSON);
					...
					newObj.put("totPayable",amount + totalTaxAmt + totalItemAdjAmt + subsidyAmt + totalDiscAmt + serviceChg);
						 ...
						 if(scheme == "MGPS_10Pecent" && subsidyAmt != 0)
