		...
		if(UtilValidate.isNotEmpty(vehicleTrpStatList)){
			for(i=0;i<shipmentIds.size();i++){
				...
				if(UtilValidate.isNotEmpty(tempVehicleTripStatusList)){
					...
					List<GenericValue> crateCanAcctList = delegator.findList("CrateCanAccount", EntityCondition.makeCondition("shipmentId", EntityOperator.EQUALS, shipmentId),null,null,null, false);
					
					GenericValue crateCanAcct = null;
					if(UtilValidate.isNotEmpty(crateCanAcctList)){
						crateCanAcct=EntityUtil.getFirst(crateCanAcctList);
					}
