...
function makeDatePicker(fromDateId ,thruDateId){
            ...
            $("#wizard-2").steps({
                ...
                {	
                	if(currentIndex == 0 && newIndex == 1){
                		var billToPartyId = $("#0_lookupId_billToPartyId").val();  
                		
                		if( (billToPartyId).length < 1 ) {
					    	$('#billToPartyId').css('background', 'yellow'); 
					       	setTimeout(function () {
					           	$('#billToPartyId').css('background', 'white').focus(); 
					       	}, 800);
					    	return false;
				    	}
                		billToPartyName = partyNameObj[billToPartyId];
                        if( (billToPartyName).length < 1 ) {
 					    	$('#billToPartyName').css('background', 'yellow'); 
 					       	setTimeout(function () {
 					           	$('#billToPartyName').css('background', 'white').focus(); 
 					       	}, 800);
 					    	return false;
				    	}
                ...
                {
				    processSOEntryInternal("CreateTSSO", "CreateTankerSaleSO");
                	form.submit();
                }
            });
	}); 
	...
	$(document).ready(function(){
		...
		$("#billToPartyId").autocomplete({ source: partyAutoJson }).keydown(function(e){
	...
	function dispSuppName(selection){
	   var value = $("#billToPartyId").val();
	   billToPartyName = partyNameObj[value];
	   $("#billToPartyName").html("<h4>"+billToPartyName+"</h4>");
	}    
...
<#if orderEditParam?has_content>
	...
	<#assign shipmentId = orderInfo.shipmentId>
	<#assign shipment = delegator.findByAnd("Shipment", {"shipmentId" : shipmentId})>
	
	<#if shipment!= "undefined" && shipment?has_content>
		<#assign ShipToPartyId = shipment[0].partyIdTo?if_exists>
	</#if>
	
</#if>
	
<form id="CreateTSSO"  action="<@ofbizUrl>CreateTankerSaleSO</@ofbizUrl>" name="CreateTSSO" method="post">

<div id="wizard-2">
    <h3>SO Information</h3>
    <section>
      <fieldset>
            <table cellpadding="10" cellspacing="10" class='h3' width="50%">
                   <tr>
						<td class="label">Order Type(<font color="red">*</font>) :</td>
					    <td class="label">
						<select name="orderTypeId" id="orderTypeId" onchange="javascript: hideExtSO();">
					...
					<tr>
                        ...
                        <#else><td class="label"><b>SO Date : </b></td> 
                        </#if> 
					    <td>
					    	<#if orderId?exists && orderInfo.get("orderDate")?exists>
					    		<input type="text" name="orderDate" id="orderDate" size="26" maxlength="60" autocomplete="off" value="${orderInfo.get("orderDate")?if_exists}"/>
					    	<#else>
						      	<input type="text" name="orderDate" id="orderDate" size="26" maxlength="60" autocomplete="off"/>
					      	</#if>
        				 </td>
					...
					<tr>
						...
						<td>
							<#if orderId?exists && orderInfo.get("validFromDate")?exists>
					    		<input type="text" name="fromDate" id="fromDate" size="26" maxlength="60" autocomplete="off" value="${orderInfo.get("validFromDate")?if_exists}"/>
					    	<#else>
					    		<input type="text" name="fromDate" id="fromDate" size="26" maxlength="60" autocomplete="off"/>
					    	</#if>
						</td>
					</tr>
					<tr>
						...
						<td>
							<#if orderId?exists && orderInfo.get("validThruDate")?exists>
					    		<input type="text" name="thruDate" id="thruDate" size="26" maxlength="60" autocomplete="off" value="${orderInfo.get("validThruDate")?if_exists}"/>
					    	<#else>
					    		<input type="text" name="thruDate" id="thruDate" size="26" maxlength="60" autocomplete="off"/>
					    	</#if>
						</td>
					</tr>
					</#if>
					...
					<#if orderId?exists && quoteInfo.get("quoteIssueDate")?exists>
      		        ...
      		        <tr>
					    ...
					    <td class="label">
					    	<#if orderId?exists && orderInfo.get("billToPartyId")?exists>
					    		...
					    		<@htmlTemplate.lookupField value="${billToPartyId?if_exists}" formName="CreateTSSO" size="10" maxlength="60" name="billToPartyId"  id="billToPartyId" fieldFormName="LookupPartyName"/>
					    	<#else>
					      		<@htmlTemplate.lookupField  formName="CreateTSSO" size="26" maxlength="60" name="billToPartyId" id="billToPartyId" fieldFormName="LookupPartyName"/>
      		                </#if>
      		                
					    </td>
					    <td class="label">
					    	<span class="tooltip">Invoice will be raised against this Party </span>
					    </td>
					 </tr>
					 ...
					 <tr>
					    ...
					    <td class="label">
					    	<#if ShipToPartyId?exists>
					    	<@htmlTemplate.lookupField value="${ShipToPartyId?if_exists}" formName="CreateTSSO" size="26" maxlength="60" name="ShipToPartyId"  id="ShipToPartyId" fieldFormName="LookupPartyName"/>
					    	<#else>
					      	<@htmlTemplate.lookupField  formName="CreateTSSO" size="26" maxlength="60" name="ShipToPartyId" id="ShipToPartyId" fieldFormName="LookupPartyName"/>
      		                </#if>
					    </td>
					 </tr>
					 <tr>
					    ...
					    <td>
					    	<#if orderId?exists && orderInfo.get("refNo")?exists>
					    		<input type="text" name="refNo" id="refNo" size="26" maxlength="60" autocomplete="off" value="${orderInfo.get("refNo")?if_exists}"/>
					    	<#else>
						      	<input type="text" name="refNo" id="refNo" size="26" maxlength="60" autocomplete="off"/>
					      	</#if>
					...
					<tr>
					    ...
					    <td>
					    	<#if orderId?exists && orderInfo.get("estimatedDeliveryDate")?exists>
					    		<input type="text" name="estimatedDeliveryDate" id="estimatedDeliveryDate" size="26" maxlength="60" autocomplete="off" value="${orderInfo.get("estimatedDeliveryDate")?if_exists}"/>
					    	<#else>
					    		<input type="text" name="estimatedDeliveryDate" id="estimatedDeliveryDate" size="26" maxlength="60" autocomplete="off"/>
						   	</#if> 
        				 </td>
					</tr>
					<tr>
					    <td class="label"><b>ARC No / CPC No / Ext. SO :</b></td>
					    <td>
					    	<#if orderId?exists && orderInfo.get("SONumber")?exists>
					    		<input type="text" name="SONumber" id="SONumber" size="26" maxlength="60" autocomplete="off" value="${orderInfo.get("SONumber")?if_exists}"/>
					    	<#else>
					    		<input type="text" name="SONumber" id="SONumber" size="26" maxlength="60" autocomplete="off"/>
					    	</#if>
        				 </td>
					</tr>
					<tr>
					    ...
					    <td>
					    	<#if orderId?exists && orderInfo.get("fileNo")?exists>
					    		<input type="text" name="fileNo" id="fileNo" size="26" maxlength="60" autocomplete="off" value="${orderInfo.get("fileNo")?if_exists}"/>
					    	<#else>
					    		<input type="text" name="fileNo" id="fileNo" size="26" maxlength="60" autocomplete="off"/>
					    	</#if>
			 			...
			 			<div class="screenlet-body" id="FieldsDIV" >
			 				<table width="100%" border="0" cellspacing="10" cellpadding="10">
				 		        <tr>
						        	...
						        	<td align="center"><h2><font color="black">Total SO Value :</font> <font color="green"><span id="totalSOAmount">Rs. 0</span></font></h2></td>
						        	<td align='right'><input class="styled-button" type="button" id="calculateBtn"  name="calculateBtn" value="Calculate" onClick="javascript: calculateSOValue();"/></td>
				 		         </tr>
				        	</table>
