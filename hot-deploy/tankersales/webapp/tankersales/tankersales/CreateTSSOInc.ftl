...
<script type="application/javascript">
	...
	function processSOEntryInternal(formName, action) {
		...
		<#if changeFlag?exists && changeFlag != "AdhocSaleNew">
			...
			var party = jQuery("<input>").attr("type", "hidden").attr("name", "billToPartyId").val(billToPartyId);
		    ...
		    var SOField = jQuery("<input>").attr("type", "hidden").attr("name", "SONumber").val(soNumber);
			...
			jQuery(formId).append(jQuery(SOField));
	...
	function processIndentEntry(formName, action) {
		...
		processSOEntryInternal(formName, action);
    ...
    function updateGridAmount(){
		...
		if(amt > 0 ){
			var dispText = "<b>  [Total SO Amt: Rs " +  amt + "]</b>";
		}
		else{
			var dispText = "<b>  [Total SO Amt: Rs 0 ]</b>";
		}
    ...
    function updateInvoiceTotalAmount(){
		...
		function updateProductTotalAmount() {
			...
			if(amt > 0 ){
				var dispText = "<b> [Total SO Value: Rs" +  amt + "]</b>";
			}
			else{
				var dispText = "<b>[Total SO Value: Rs 0 ]</b>";
			}
	...
	jQuery(function(){
	     var partyId=$('[name=billToPartyId]').val();
		 if(partyId){
		    gridShowCall();
	     }else{ 
	        gridHideCall();
