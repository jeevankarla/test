...
<script type="application/javascript">
	...
	function processIndentEntryInternal(formName, action) {
		...
		var partyTo = jQuery("<input>").attr("type", "hidden").attr("name", "partyId").val(partyId);
		var partyFrom = jQuery("<input>").attr("type", "hidden").attr("name", "partyIdFrom").val(partyIdFrom);
		jQuery(formId).append(jQuery(partyTo));
		jQuery(formId).append(jQuery(partyFrom));
	...
	jQuery(function(){
		 ...
		 var partyIdTo=$('[name=partyIdTo]').val();
		 if(partyId){
		 	setupGrid1();
	     }
