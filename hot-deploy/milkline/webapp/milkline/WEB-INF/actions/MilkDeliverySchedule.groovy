...
List<GenericValue> productValuesList =FastList.newInstance();
...
productValuesList =delegator.findList("ProductAndCategoryMember", discontinuationDateCondition,null, ["sequenceNum"], null, false);

context.productValuesList =productValuesList;

context.productList = EntityUtil.filterByCondition(productList, EntityCondition.makeCondition("productId",EntityOperator.NOT_IN , productValuesList.productId));
...
if (checkListItemList.size() > 0) {		
	...
	productValuesList.each{ products ->
		productsSubMap[products.productId] = '';
	}
	checkListReportList.add(lastChangeSubProdMap);
	lastChangeSubProdList.add(lastChangeSubProdMap);
	productsReportList.add(productsSubMap);
	ChangeProductsList.add(productsSubMap);
}	
...
List templist = [];
 ...
 productValuesList.each{ products ->
	 productTotalsMap[products.productId] =BigDecimal.ZERO;
 }
...
for (int i=0; i < checkListItemList.size(); i++) {
	...
	if (tempOrderId == "") {
		...
		productsSubMap["boothId"] = checkListItemProd.originFacilityId;
		productsSubMap["supplyType"] = supplyTypeMap[checkListItemProd.productSubscriptionTypeId];
		productsSubMap["routeId"] = checkListItemProd.parentFacilityId;
		
		Map<String, Object> boothPayments = NetworkServices.getBoothPayments(delegator, dctx.getDispatcher(), userLogin, UtilDateTime.toDateString(dayBegin, "yyyy-MM-dd HH:mm:ss"), null, checkListItemProd.originFacilityId ,null ,Boolean.FALSE);
		Map<String, Object> currentBoothPayments = NetworkServices.getBoothReceivablePayments(delegator, dctx.getDispatcher(), userLogin, UtilDateTime.toDateString(dayBegin, "yyyy-MM-dd HH:mm:ss"), null, checkListItemProd.originFacilityId ,null ,Boolean.TRUE, Boolean.FALSE);
		...
		productsSubMap["PREV_DUE"] = ((BigDecimal)boothTotalDues.getAt("totalDue")).subtract((BigDecimal)boothTotalDues.getAt("grandTotal"));
		productsSubMap["TODAY_DUE"] = (BigDecimal)currentBoothTotalDues.getAt("grandTotal");
		
		}
	if(tempBoothId != checkListItemProd.originFacilityId){
		scheduleListReportMap.putAt(tempBoothId, lastChangeSubProdList);
		productsReportMap.putAt(tempBoothId, ChangeProductsList);
		tempBoothId = checkListItemProd.originFacilityId;
		lastChangeSubProdList = FastList.newInstance();
		ChangeProductsList = FastList.newInstance();
	}	
	...
	productsSubMap[checkListItemProd.productId] = (checkListItemProd.quantity).setScale(1, rounding);	
...
context.productsReportMap =productsReportMap;
context.ChangeProductsList =ChangeProductsList;
