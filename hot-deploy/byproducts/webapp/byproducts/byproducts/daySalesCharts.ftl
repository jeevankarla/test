	...
	function setupGrid1() {
        ...
        <#if productReportList?exists> 
	        <#list productReportList as productReport>				
				{id:"${productReport.name}" , name:"${productReport.name}", totalQuantity:"${productReport.totalQuantity}"}<#if productReport_has_next>,</#if>
			</#list>			
		</#if>		
		];
		var dataView;
		...
		var columns = [
			...
			{id:"quantity", name:"QUANTITY (in Units)", field:"totalQuantity", width:100, minWidth:70, cssClass:"cell-title-right", sortable:false}	
		];
...
$(document).ready(function(){
  var data = [
<#list productReportList as productReport>
    {label: "${StringUtil.wrapString(productReport.name?default(""))}", data: ${productReport.totalQuantity?if_exists}}<#if productReport_has_next>,</#if>
</#list>    
  ];
...
<div class="screenlet">
    ...
    <div class="screenlet-body">
		...
		<div id="myGrid1" style="width:300px;height:250px"></div>   		
    </div>
</div>
</div>
<#else>
