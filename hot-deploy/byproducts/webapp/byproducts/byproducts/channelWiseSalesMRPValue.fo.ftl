...
<#escape x as x?xml>
    <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
        ...
        ${setRequestAttribute("OUTPUT_FILENAME", "ChannelWise Sales(SALE value).txt")}
       <#if periodTotalsMap?has_content>        
		        ...
		        <fo:page-sequence master-reference="main" font-size="10pt">	
		        	<fo:static-content flow-name="xsl-region-before" font-family="Courier,monospace">
                    	...
                    	<fo:block text-align="left" font-size="7pt" keep-together="always"  white-space-collapse="false">TIN ::                                                           PRODUCTS CHANNELWISE SALE VALUE STATEMENT FOR THE MONTH OF :: ${parameters.customTimePeriodId}</fo:block>
										...
										<fo:table-row>
				                        	...
				                        	<#list facilityCategoryList as facilityCategory>
				                        		<#if (facCatTotals.get(facilityCategory)?has_content)>
				                        			...
				                        			<fo:table-cell>
				                            			<fo:block  keep-together="always" text-align="right" font-size="7pt" white-space-collapse="false">${categoryWiseTotals.get("MRPTotalValue")?if_exists?string("#0.00")}</fo:block>  
				                        			</fo:table-cell>
				                        			<#assign totalQty = totalQty + (categoryWiseTotals.get("quantity"))>
				                        			<#assign MRPTotalValue = MRPTotalValue + (categoryWiseTotals.get("MRPTotalValue"))>
				                        		<#else>
				                        			<fo:table-cell>
				                        	...
				                        	<fo:table-cell>
				                            	<fo:block  keep-together="always" text-align="right" font-size="7pt" white-space-collapse="false">${MRPTotalValue?if_exists?string("#0.00")}</fo:block>  
				                        	</fo:table-cell>
										</fo:table-row>
										<fo:table-row>
	                    					<fo:table-cell>
	                	...
	                	<#--	<fo:table-row>
		                		...
		                		<#list facilityCategoryList as facilityCategory>
		                			<#if (grandTotalsMap.get(facilityCategory)?has_content)>
	                        			...
	                        			<#assign grandTotalValue = grandTotalValue + (categoryWiseTotals.get("MRPTotalValue"))>
	                        		<#else>
	                        			<fo:table-cell>
	                            			<fo:block  keep-together="always" text-align="right" font-size="7pt" white-space-collapse="false"></fo:block>  
	                        			</fo:table-cell>
	                        	...
	                        	<fo:table-cell>
                            			<fo:block  keep-together="always" text-align="right" font-size="7pt" white-space-collapse="false">${grandTotalQty}</fo:block>  
                        		</fo:table-cell>
                        		<fo:table-cell>
