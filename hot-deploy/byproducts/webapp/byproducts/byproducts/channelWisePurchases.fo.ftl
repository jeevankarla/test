...
<#escape x as x?xml>
    <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
        ...
        ${setRequestAttribute("OUTPUT_FILENAME", "ChannelPurchases.txt")}
       <#if periodTotalsMap?has_content> 
		        <fo:page-sequence master-reference="main" font-size="10pt">	
		        	<fo:static-content flow-name="xsl-region-before" font-family="Courier,monospace">
                    	...
                    	<fo:block text-align="left" font-size="7pt" keep-together="always"  white-space-collapse="false">TIN ::                                                           PRODUCTS CHANNELWISE PURCHASES STATEMENT FOR THE MONTH OF :: ${parameters.customTimePeriodId}</fo:block>
            	...
            	<fo:block>
            		...
            		<fo:table-column column-width="40pt"/>	
	                    <fo:table-body>
	                    	...
	                    	<#assign totalQty = 0>
							<#assign basicValue = 0>
	                    	<#list unionCategoriesList as unionCategory>
	                    		<#if (periodTotalsMap.get(unionCategory)?has_content)>
	                    			<#assign periodProductTotals = (periodTotalsMap.get(unionCategory)).entrySet()>
	                    			<#list periodProductTotals as periodProductTotalsEntry>
	                    			<#if periodProductTotalsEntry.getKey() == "prodCategoryTotals">
	                    				<#assign purTotalsMap = (periodProductTotalsEntry.getValue()).get("purchase")>
	                    				<#assign facCatTotals = (purTotalsMap).get("CategoryWiseTotals")>
	                    				<fo:table-row>
	                    					<fo:table-cell>
		                            			<fo:block font-size="7pt" align-text="left">=======================================================================================================================================================================================================================================</fo:block>  
		                        			</fo:table-cell>
										...
										<fo:table-row>
				                        	...
				                        	<fo:table-cell>
				                            	<fo:block  keep-together="always" text-align="right" font-size="7pt" white-space-collapse="false">${totalQty}</fo:block>  
				                        	</fo:table-cell>
				                        	<fo:table-cell>
				                                         <fo:block text-align="right" font-size="7pt" keep-together="always" font-family="Courier,monospace" white-space-collapse="false">&#160;</fo:block> 
				                        	</fo:table-cell>
	                    			...
	                    			<#else>
	                    				<#assign purchaseMap = (periodProductTotalsEntry.getValue()).get("purchase")>
			                        	<#assign purchaseTotals = (purchaseMap).get("CategoryWiseTotals")>
			                    		<fo:table-row>
			                    			<#assign totalQty = 0>
											<#assign basicValue = 0>
			                    			<fo:table-cell>
				                            	<fo:block  keep-together="always" text-align="right" font-size="7pt" white-space-collapse="false">${serialNo}</fo:block>  
				                        	</fo:table-cell>
			                    			<fo:table-cell>
				                            	<fo:block  keep-together="always" text-align="right" font-size="7pt" white-space-collapse="false">${periodProductTotalsEntry.getKey()}</fo:block>  
				                        	</fo:table-cell>
				                        	<#assign productEnt = delegator.findOne("Product", {"productId" : periodProductTotalsEntry.getKey()}, true)>
				                        	<fo:table-cell>
	                            				<fo:block text-align="right" keep-together="always" font-size="7pt"  white-space-collapse="false">${Static["org.ofbiz.order.order.OrderServices"].nameTrim((StringUtil.wrapString(productEnt.brandName)),12)}</fo:block>        
	                        				</fo:table-cell>
				                        	<#list facilityCategoryList as facilityCategory>
				                        		<#if (purchaseTotals.get(facilityCategory)?has_content)>
				                        			<#assign categoryWiseTotals = (purchaseTotals).get(facilityCategory)>
				                        			<fo:table-cell>
				                            			<fo:block  keep-together="always" text-align="right" font-size="7pt" white-space-collapse="false">${categoryWiseTotals.get("quantity")}</fo:block>  
				                        			</fo:table-cell>
				                        			<fo:table-cell>
