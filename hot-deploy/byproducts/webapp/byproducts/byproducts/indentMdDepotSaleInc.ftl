...
<script type="application/javascript">
	...
	var productTaxListMap=${StringUtil.wrapString(productTaxListJSON)!'[]'};
	...
	function setupGrid1() {
        ...
        grid.onCellChange.subscribe(function(e,args) {
        	if (args.cell == 2) {
				...
				//set basicPrice and tax price
				var basicPrice=0;
				var taxList=productTaxListMap[prod];
				//alert("==taxList==IN==2="+taxList+"===price=="+price+"===UDP=="+udp);
				if(taxList != 'undefined' && taxList != null ){
				basicPrice=price;
					for(var i=0 ; i<taxList.length ; i++){
					   var innerTaxMap=taxList[i];	
						if(innerTaxMap != 'undefined' && innerTaxMap != null && "VAT_SALE"==innerTaxMap["taxType"] ){
						//alert("==innerTaxMap==="+innerTaxMap["taxType"]);
						 data[args.row]['vatPrice']=innerTaxMap["amount"];
						 if(basicPrice>0){
						 basicPrice=basicPrice-innerTaxMap["amount"];
						 }
						data[args.row]['vatPercent']=innerTaxMap["percentage"];
						}
						if(innerTaxMap != 'undefined' && innerTaxMap != null && "CST_SALE"==innerTaxMap["taxType"] ){
						//alert("==innerTaxMap==="+innerTaxMap["taxType"]);
						 data[args.row]['cstPrice']=innerTaxMap["amount"];
						 if(basicPrice>0){
						 basicPrice=basicPrice-innerTaxMap["amount"];
						 }
						data[args.row]['cstPercent']=innerTaxMap["percentage"];
						}
					}//for end  
					
				 }//end of check
				  data[args.row]['basicPrice']=basicPrice;
				if(isNaN(qty)){
					qty = 0;
				}
				var roundedAmount;
