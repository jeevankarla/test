                 ...
                 <fo:block font-size="7pt">
                    ...
                    <fo:table-column column-width="45pt"/>
		          	<fo:table-header border-width="1pt" border-style="dotted">
		                ...
		                <fo:table-cell><fo:block text-align="left" keep-together="always" white-space-collapse="false">.                                                    DETAILS OF RECOVERIES</fo:block><fo:block font-size="7pt" text-align="left">--------------------</fo:block></fo:table-cell>
		               	<#list orderAdjItemsList as orderAdjItems>
		               	<fo:table-cell><fo:block font-size="7pt" linefeed-treatment="preserve">&#xA;</fo:block><fo:block font-size="7pt" text-align="left">--------------------</fo:block><fo:block font-size="7pt" text-align="right" >${orderAdjItems.description?if_exists}</fo:block><fo:block font-size="7pt" text-align="right">AMOUNT</fo:block></fo:table-cell>
		                </#list>
                    ...
                    <fo:table-body>
	                    ...
	                    <#if unitData.getKey() == "TOTAL">
            				...
            				<fo:table-row>	
            					<fo:table-cell >	
	                        		<fo:block font-size="7pt" text-align="left" >${unitAmountAbstractData.get("unitName")?if_exists}</fo:block>
	                        	</fo:table-cell>
	                        	<fo:table-cell >	
	                        		<fo:block font-size="7pt" linefeed-treatment="preserve">&#xA;</fo:block>
	                        	</fo:table-cell>
	                        	<#list procurementProductList as procProducts>
	                        	<fo:table-cell >
	                        		<fo:block text-align="right" >${unitAmountAbstractData.get(procProducts.brandName+"Amount")?if_exists?string("##0.00")}</fo:block>
	                        	</fo:table-cell>
								</#list>	                        	
	                            <fo:table-cell >	
	                        		<fo:block font-size="7pt" text-align="right" >${unitAmountAbstractData.get("opCost")?if_exists?string("##0.00")}</fo:block>
	                        	</fo:table-cell>
	                        	<fo:table-cell >	
	                        		<fo:block font-size="7pt" text-align="right" >${unitAmountAbstractData.get("cartage")?if_exists?string("##0.00")}</fo:block>
	                            </fo:table-cell> 
	                        	<fo:table-cell >	
	                            	<fo:block font-size="7pt" text-align="right" keep-together="always">${unitAmountAbstractData.get("AddnTot")?if_exists?string("##0.00")}</fo:block> 
	                            </fo:table-cell>     
	                            <fo:table-cell >	
	                            	<fo:block font-size="7pt" text-align="right">${unitAmountAbstractData.get("grossAmount")?if_exists?string("##0.00")}</fo:block>                               
	                            </fo:table-cell> 
	                            <fo:table-cell >	
	                            ...
	                            <fo:table-cell >	
	                            	<fo:block font-size="7pt" text-align="right">${deductionType?if_exists?string("##0.00")}</fo:block>                               
	                            </fo:table-cell>
	                            <#assign deductionType = 0>
	                             </#list>
	                        	<fo:table-cell >	
	                            	<fo:block font-size="7pt" text-align="right" keep-together="always">${unitAmountAbstractData.get("DednsTot")?if_exists?string("##0.00")}</fo:block> 
	                            </fo:table-cell>     
	                        	<fo:table-cell >	
	                            	<fo:block font-size="7pt" text-align="right" keep-together="always">${unitAmountAbstractData.get("netAmount")?if_exists?string("##0.00")}</fo:block> 
	                            </fo:table-cell>     
	                            <fo:table-cell >	
	                            	<fo:block font-size="7pt" text-align="right" keep-together="always">${unitAmountAbstractData.get("tipAmount")?if_exists?string("##0.00")}</fo:block> 
	                            </fo:table-cell> 
				            </fo:table-row>	    
				            </#list> 
                    </fo:table-body>
