...
$(document).ready(function(){
    ...
    jQuery.plot($("#graph1"), [{data: ${StringUtil.wrapString(lowProcDataListJSON)},
		points: { show: false },
									bars: {show: true,
                   						   	barWidth: 0.2,
                   							points: { show: false }, 
                   							align: 'center'}},
								  {data: ${StringUtil.wrapString(lowProcFatDataListJSON)}, label: "Fat",  xaxis: 1, yaxis:2, lines: { show: true } }, 
							  	  {data: ${StringUtil.wrapString(lowProcSnfDataListJSON)}, label: "Snf", xaxis: 1, yaxis:2, lines: { show: true } }],
				{ 
					series: {
                   		points: { show: true }                   				
                	}, 
     				...
     				xaxis: {
         				min: 0,
         				max : ${lowProcDataListJSON.size()} + 1,
         				ticks:${StringUtil.wrapString(lowProclabelsJSON)},
         				rotateTicks: 140
     				},                   	         		             		            		
				});
...
<div class="screenlet">
    ...
    <#if facility?has_content>    
      	<h3>Procurement Comparison Data for ${facility.facilityName?if_exists} (${parameters.facilityId?if_exists}) [${parameters.procurementDate?if_exists}-${parameters.procurementThruDate?if_exists}] </h3>
	</#if>
    </div>
    <div class="screenlet-body">
    	
    	<h1>Good Performance  (Fat>4.5)</h1>
   		<div id="graph" class="graph" title = "Good Performance"></div>
        <br><br><br><br>   		   		
        <h1>Poor Performance (Fat<4.5)</h1>
        <div id="graph1" class="graph1" title = "Poor Performance"></div>
    </div>   
</div>
