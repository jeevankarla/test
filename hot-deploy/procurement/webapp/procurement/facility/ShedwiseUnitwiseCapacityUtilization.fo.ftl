						 		...
						 		<fo:table-column column-width="40pt"/>
		   						<fo:table-body>
		   							...
		   							<#list shedValues as shedDetails>
		   								...
		   								<#if shedNameMap?has_content>
		   									<#assign shedNameValues=shedNameMap.entrySet()>
		   									<#list shedNameValues as shedNameList>
		   										<#if shedNameList.getKey()==shedDetails.getKey()>
		   											<#assign shedName = (shedNameList.getValue()).toUpperCase()>
										...
										<#assign shedtotGEVariation=0>
							           	<fo:table-row>
											<#if shedNameMap?has_content>
		   									<#assign shedNameValues=shedNameMap.entrySet()>
			   									<#list shedNameValues as shedNameList>
			   										<#if shedNameList.getKey()==shedtotDetails.getKey()>
			   											<#assign shedName = (shedNameList.getValue()).toUpperCase()>
								           				<#assign shedName = shedName.substring(0,shedName.indexOf("MILK SHED"))>
														<fo:table-cell>
															<fo:block keep-together="always" text-align="left" font-size="5pt"> ${shedName}</fo:block>
