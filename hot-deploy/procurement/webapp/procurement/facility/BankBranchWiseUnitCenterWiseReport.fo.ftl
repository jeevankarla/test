<#escape x as x?xml>
    <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
        ...
        ${setRequestAttribute("OUTPUT_FILENAME", "bankBranchUnitCenterWiseReport.txt")}
		...
		<#else>
	<#if bankBranchUnitCenterWiseMap?has_content>
		<#assign bankWiseDetailsList = bankBranchUnitCenterWiseMap.entrySet()>	
		<#assign bankAmount=0>	
		
		<#list bankWiseDetailsList as bankWiseDetails>
			<#assign branchWiseDetailsList = bankWiseDetails.getValue().entrySet()>
			<#assign branchAmount=0>
			<#list branchWiseDetailsList as branchWiseDetails>
				<fo:page-sequence master-reference="main">
				<fo:static-content flow-name="xsl-region-before" font-family="Courier,monospace" >
					...
					<fo:block text-align="left" white-space-collapse="false" keep-together="always" >SHED : ${facilityDetails.facilityName?if_exists}   NAME OF THE BANK : ${bankWiseDetails.getKey()}  BRANCH NAME : ${branchWiseDetails.getKey()} </fo:block>
					<#if transferBankMap?has_content>
						<#assign bankName = bankWiseDetails.getKey()>
						<#assign transferBankDetails = transferBankMap.get(bankName)>
						<#if transferBankDetails?has_content>
							<#assign trnBranchDetails  = transferBankDetails.get(branchWiseDetails.getKey())>
							<#if trnBranchDetails?has_content>
								<fo:block text-align="left" white-space-collapse="false" keep-together="always" font-size="8pt">Trns BANK: ${trnBranchDetails.TrnsferBankName}       BRANCH:${trnBranchDetails.TrnsferBranch}      IFSC:${trnBranchDetails.TrnsfIfcNo}    A/C :${trnBranchDetails.TrnsfAcNo}</fo:block>			
							</#if>
						</#if>
					</#if>
				...
				<fo:flow flow-name="xsl-region-body" font-family="Courier,monospace">
					<#assign unitWiseDetailsList= branchWiseDetails.getValue().entrySet()>	
					<#assign unitAmount=0>
					<#assign sno=0>
					<#assign printLine=0>
					<#assign tempPage = 0>
					<#list unitWiseDetailsList as unitDetails>
						...
						<#if unitAmount !=0>
						<#assign branchAmount=branchAmount+unitAmount>
							...
							<fo:table>
								...
								<fo:table-body>
									<fo:table-row>
										...
										<fo:table-cell>
											<fo:block text-align="right">  ${branchAmount?if_exists?string("#0.00")}</fo:block>
										</fo:table-cell>
									</fo:table-row>
								</fo:table-body>
							</fo:table>
