...
orderItemsList = delegator.findList("OrderHeaderItemProductAndFacility",condition,null,["facilityCode" ,"estimatedDeliveryDate","changeDatetime"],null,false);
orderItemsListSorted = [];
if(UtilValidate.isNotEmpty(orderItemsList)){ 
	orderItemsList.each{orderItem->
		orderItem.set("facilityCode", (orderItem.facilityCode).toInteger());
		orderItemsListSorted.add(orderItem);
	}
	orderItemsListSorted = UtilMisc.sortMaps(orderItemsListSorted, UtilMisc.toList("facilityCode"));
}

JSONArray orderItemsJSON = new JSONArray();
if(UtilValidate.isNotEmpty(orderItemsListSorted)){
	i=0;
	orderItemsListSorted.each{ orderItem->
		++i;
		...
		newObj.put("productName",orderItem.productName);
