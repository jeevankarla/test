<#escape x as x?xml>
    <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
        ...
        ${setRequestAttribute("OUTPUT_FILENAME", "UnionOrPrivateDairiesAbstSupport.txt")}
		...
		<#else>
		 ...
		 <#if finalAbstractSupportList?has_content>
		    <#list finalAbstractSupportList as finalAbstractSupport>
		 	<#assign finalBillingList = finalAbstractSupport.get("finalBillingList")>
		 	<#assign productId = finalAbstractSupport.get("productId")>
		 	<#assign opCost = finalAbstractSupport.get("opCost")>
		 	<#assign facilityId = finalAbstractSupport.get("unitId")>
		 	<#assign reportTypeStr = finalAbstractSupport.get("reportTypeStr")>
		 	<#assign uomTypeStr = finalAbstractSupport.get("uomTypeStr")>
			...
			<fo:page-sequence master-reference="main">
				<fo:static-content flow-name="xsl-region-before" font-family="Courier,monospae">
					...
					<fo:block text-align="left" white-space-collapse="false" font-size="6pt" keep-together="always">&#160; UNIT NAME  :  ${facility.getString("mccCode")?if_exists}:${facility.getString("facilityName")?if_exists}                MILK BILLS        ${product.brandName?if_exists}                       </fo:block>
