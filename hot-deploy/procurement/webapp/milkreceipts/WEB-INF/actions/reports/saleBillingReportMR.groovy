...
/*	partyPostalAddress= dispatcher.runSync("getPartyPostalAddress", [partyId:partyId, userLogin: userLogin]);
	address1="";address2="";city="";postalCode="";
	 ...
	 tinCstKstDetails = delegator.findList("PartyIdentification",EntityCondition.makeCondition("partyId", EntityOperator.EQUALS ,partyIdTo)  , null, null, null, false );
...
conditionList.add(EntityCondition.makeCondition("purposeTypeId", EntityOperator.EQUALS , "OUTGOING"));
...
if(UtilValidate.isNotEmpty(productIds)){
	productIds.each{eachProductId->
		...
		priceChart = PriceServices.fetchPriceChartForParty(dctx, [priceDate:dayBegin, partyId: partyIdTo, userLogin: userLogin,]);
	...
	/*	conditionList.clear();
		...
		if(UtilValidate.isNotEmpty(prodMilkTransferList)){
			...
			prodMilkTransferList.each{eachDateMilkReeceipt->
				...
				BigDecimal receivedQuantity=eachDateMilkReeceipt.quantity;
