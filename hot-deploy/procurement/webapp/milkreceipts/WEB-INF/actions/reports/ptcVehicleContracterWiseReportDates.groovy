...
Map totContractSubTotMap=FastMap.newInstance();
	...
	milkTransferList = delegator.findList("MilkTransfer", condition, null, orderBy, null, false);
	if(UtilValidate.isNotEmpty(milkTransferList)){
		...
		conditionList.clear();
		conditionList.add(EntityCondition.makeCondition("vehicleId", EntityOperator.IN , containerIds));
		conditionList.add(EntityCondition.makeCondition("incidentDate", EntityOperator.GREATER_THAN_EQUAL_TO,dayBegin));
		conditionList.add(EntityCondition.makeCondition("incidentDate", EntityOperator.LESS_THAN_EQUAL_TO,dayEnd));
		condition=EntityCondition.makeCondition(conditionList,EntityOperator.AND);
