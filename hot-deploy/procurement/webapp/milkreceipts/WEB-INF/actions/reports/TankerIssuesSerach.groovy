 ...
 */
 ...
 if(UtilValidate.isNotEmpty(hideSearch) && (hideSearch.equalsIgnoreCase("N"))){
	 ...
	 milkTransferDetailsList = EntityUtil.filterByCondition(vehicleTripStatusViewList, EntityCondition.makeCondition(EntityCondition.makeCondition("estimatedEndDate", EntityOperator.EQUALS , null ),EntityOperator.OR,EntityCondition.makeCondition("vehicleTripStatusId", EntityOperator.EQUALS , "MR_ISSUE_GRWEIGHT" )));
 }
 ...
 def getShiftWiseRecords(Timestamp shiftDateTimeStart,Timestamp shiftDateTimeEnd){
	   ...
	   shiftList.each{eachShift->
		   ...
		   shiftsMap.put("fat",eachShift.fat);
		   shiftsMap.put("snf",eachShift.snf);
		   ...
		   eachCondList.add(EntityCondition.makeCondition("vehicleTripStatusId", EntityOperator.EQUALS , "MR_ISSUE_INIT" ));
		   ...
		   if(vehicleEntryProductList){
			   ...
			   if(UtilValidate.isNotEmpty(vehicleEntryProduct.estimatedEndDate)){
				   receiveDate = UtilDateTime.toDateString(vehicleEntryProduct.estimatedEndDate,"dd-MM-yyyy HH:mm");
			   }
