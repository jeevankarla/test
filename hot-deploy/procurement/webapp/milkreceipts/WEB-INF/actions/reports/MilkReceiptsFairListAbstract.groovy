	...
	unitMccList.each{unitId->
		if(UtilValidate.isEmpty(facilityWiseMap.get(unitId))){
			...
			if(UtilValidate.isNotEmpty(mccTypeId)){
				periodTotals = MilkReceiptReports.getMilkReceiptPeriodTotals(dctx , [fromDate: fromDate , thruDate: fromDate , facilityId:unitId.facilityId]);
...
List unionOrOthersShedsCondList = FastList.newInstance(); 

unionOrOthersShedsCondList.add(EntityCondition.makeCondition("mccTypeId", EntityOperator.IN , UtilMisc.toList("UNION","OTHERS")));
unionOrOthersShedsCondList.add(EntityCondition.makeCondition("facilityTypeId", EntityOperator.EQUALS , "SHED"));
EntityCondition UOShedCondition = EntityCondition.makeCondition(unionOrOthersShedsCondList,EntityOperator.AND);


List unionAndOthersShedDetailsList = delegator.findList("Facility",UOShedCondition,null,null,null,false);
List UOShedIds = EntityUtil.getFieldListFromEntityList(unionAndOthersShedDetailsList,"facilityId",true);
//Debug.log("UOShedIds============"+UOShedIds);

finalPrivateUnitMap =[:];
conditionList =[];
conditionList.add(EntityCondition.makeCondition("parentFacilityId", EntityOperator.IN ,UOShedIds));
...
for(privateId in unionAndOthersDetailsList){
	...
	if(UtilValidate.isNotEmpty(privateId.get("mccCode"))){
...
privateIdList.each{privateUnit->
	privateTotal = MilkReceiptReports.getMilkReceiptPeriodTotals(dctx , [fromDate: fromDate , thruDate: fromDate ,facilityId: privateUnit.facilityId]);
