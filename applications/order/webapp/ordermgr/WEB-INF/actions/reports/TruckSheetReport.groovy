...
productCondition = [EntityCondition.makeCondition("enumTypeId", EntityOperator.EQUALS , "SUB_PROD_TYPE")];
productCondition.add(EntityCondition.makeCondition("enumId", EntityOperator.NOT_EQUAL , "CASH_FS"));

prodCond = EntityCondition.makeCondition(productCondition, EntityOperator.AND);
productSubscriptionTypeList = delegator.findList("Enumeration", prodCond, ["enumId"] as Set, UtilMisc.toList("sequenceId"), null, false);
...
def adjustTotalsMap(currentFacilityMap,totalsMap){	
	if(currentFacilityMap != null){		
		...
		while (treeMapIter.hasNext()) {
			...
			if(entry.getKey() != "facilityId" && entry.getKey() != "facilityType"){
...
def populateBoothOrders(boothId ,boothOrdersMap){	
	...
	fieldsToSelect = ["orderId", "orderDate", "productId", "quantity" ,"grandTotal" ,"productSubscriptionTypeId" ,"originFacilityId" ,"unitPrice" ,"categoryTypeEnum"] as Set;		
	...
	boothOrderItemsList.each { boothOrderItem ->
					...
					if(productMap[boothOrderItem.productId] == null){
						...
						typeAndCountMap["TOTAL"] += (boothOrderItem.quantity).doubleValue();
						...
						typeAndCountMap[boothOrderItem.productSubscriptionTypeId] += (boothOrderItem.quantity).doubleValue();
						typeAndCountMap["TOTAL"] += (boothOrderItem.quantity).doubleValue();
						if(boothOrderItem.productSubscriptionTypeId == "CASH"){
