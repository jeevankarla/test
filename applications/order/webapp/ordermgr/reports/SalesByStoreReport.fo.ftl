...
<#assign showProductStore = false>
<#assign showToParty = !parameters.toPartyId?has_content>
<#assign showFacility = true>
...
<#--            <#if !showProductStore><fo:block font-size="10pt">${uiLabelMap.CommonFor} ${uiLabelMap.ProductProductStore}: ${parameters.productStoreId} - ${productReportList.get(0).storeName?if_exists}</fo:block></#if>  -->
            ...
            <#if parameters.originFacilityId?has_content><fo:block font-size="10pt">${uiLabelMap.ProductLocation}: ${parameters.originFacilityId}</fo:block></#if>
            ...
            <fo:table>
                <#if showProductStore & showFacility>
                    <fo:table-column column-width="60pt"/>
                    <fo:table-column column-width="60pt"/>                    
                    <fo:table-column column-width="280pt"/>
                <#elseif showProductStore || showFacility>
                    <fo:table-column column-width="60pt"/>                   
                ...
                <fo:table-header>
                    <fo:table-row font-weight="bold">
                        ...
                        <#if showFacility><fo:table-cell border-bottom="thin solid grey"><fo:block>${uiLabelMap.ProductLocation}</fo:block></fo:table-cell></#if>
                    ...
                    <#list productReportList as productReport>
                      <#if productReport.quantityOrdered?exists && (productReport.quantityOrdered > 0)>
                        <fo:table-row>
                            ...
                            <#if showFacility>
                                <fo:table-cell padding="2pt" background-color="${rowColor}">
                                    <fo:block>${productReport.originFacilityId?if_exists}</fo:block>
                                </fo:table-cell>
                            </#if>                            
                            <fo:table-cell padding="2pt" background-color="${rowColor}">
                            ...
                            <fo:table-cell padding="2pt" background-color="${rowColor}">
                                <fo:block>${productReport.unitPrice?if_exists * productReport.quantityOrdered?if_exists}</fo:block>
                            </fo:table-cell>
                        </fo:table-row>
