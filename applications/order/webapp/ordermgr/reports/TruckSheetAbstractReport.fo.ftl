...
<fo:page-sequence master-reference="main" force-page-count="no-force">					
			<fo:static-content flow-name="xsl-region-before"> <#assign lineNumber = 5> 
				<#assign facilityNumberInPage = 0>
					<fo:block text-align="center"> ${uiLabelMap.ApDairyMsg}</fo:block>				
				...
				<#if shipment.shipmentTypeId="AM_SHIPMENT_SUPPL">          		
              	...
              	<#else>
              		<fo:block keep-together="always" font-family="Courier,monospace" white-space-collapse="false"> ${uiLabelMap.CommonPage}:<fo:page-number/>      MORNING TRUCK SHEET ABSTRACT OF ${Static["org.ofbiz.base.util.UtilDateTime"].toDateString(estimatedDeliveryDate, "dd/MM/yyyy")}</fo:block>
              	              	...
              	              	<fo:block>----------------------------------------------------------------------------------------------------------</fo:block>
            	<fo:block white-space-collapse="false" font-size="9pt"  font-family="Courier,monospace"  text-align="left">ROUTE    ${uiLabelMap.ProductTypeName}      ${uiLabelMap.TypeCredit}   ${uiLabelMap.TypeCard}  ${uiLabelMap.TypeSpecialOrder}  AGNTCS  PTCCS  TOTAL  CRATES  LITRES  AMOUNT</fo:block>
            	<fo:block>----------------------------------------------------------------------------------------------------------</fo:block>
			    </fo:static-content>
...
<#if truckSheetReportList?has_content>	
				<#list truckSheetReportList as truckSheetReport>
           			...
           			<#if (lineNumber > numberOfLines)> 
           			 ...
           			 <#if (facilityTypeId == "ROUTE" || facilityTypeId == "ZONE")>
				                ...
				                <fo:table-body>
                       			...
                       			<fo:table-row>                            
                            		...
                            		<#if facilityTypeId == "ZONE">
                            			<fo:block text-align="left">ZON TOTAL</fo:block>
                            			<#else>
                            			<fo:block text-align="left" keep-together="always">${Static["org.ofbiz.order.order.OrderServices"].nameTrim((StringUtil.wrapString(facility.get("description"))),5)}</fo:block>
                            		</#if>
                            		</fo:table-cell>
                            		<fo:table-cell>
                                		<fo:block >
			              							...
			              							<fo:table-row >                    
						                            	<fo:table-cell>
						                      			...
						                      			<#list typeEntries as typeEntry> 
						                      				...
						                      				<#if typeEntry.getKey() != "NOPKTS" && typeEntry.getKey() != "NOCRATES"> 
							                      			   <fo:table-cell >
							                      			   <#if typeEntry.getKey() == "TOTALAMOUNT">
									                            	<#assign facilityGrandTotal = (facilityGrandTotal.add(typeEntry.getValue()))>
									                            	<fo:block  text-align="right"  font-size="8pt">${typeEntry.getValue().toEngineeringString()}</fo:block>
									                            <#elseif typeEntry.getKey() != "CASH">
									                            	<#if typeEntry.getKey() == "LITRES">
									                            			<fo:block  text-align="right"  font-size="8pt">${typeEntry.getValue()?string("##0.0")}</fo:block>
									                            		<#else>
                       			...
                       			<fo:table-row >
                  					<fo:table-cell >
	                     				<fo:block >${Static["org.ofbiz.base.util.UtilDateTime"].toDateString(estimatedDeliveryDate, "dd/MM/yyyy")}</fo:block>
	                 				</fo:table-cell>
	                 				<fo:table-cell>
	                     				<fo:block></fo:block>
	                 				</fo:table-cell>	
	                 				...
	                 				<fo:table-cell >
	                     				<fo:block text-align="center">${totalCrates}-0</fo:block>
	                 				</fo:table-cell>	
	                 				<fo:table-cell>
	                     				<fo:block text-align="center">${totalLitres?string("##0.0")}*</fo:block>
	                 				</fo:table-cell>
	                  				<fo:table-cell>
	                    	 			<fo:block>${facilityGrandTotal.toEngineeringString()}*</fo:block>
	                 				</fo:table-cell>	
	            				</fo:table-row>
	            				<fo:table-row>
	            					<fo:table-cell >
            			...
            			<fo:table  table-layout="fixed" >                
                			...
                			<fo:table-row>                            
                            	...
                            	<fo:table-cell>
        							 ...
        							 <#if grandTotEntrie.getKey() != "facilityId" && grandTotEntrie.getKey() != "facilityType">
	             						...
	             						<fo:table-body> 
											...
											<#list typeEntries as typeEntry>
												<#if typeEntry.getKey() == "LITRES">
						                      	...
						                      	<#if typeEntry.getKey() == "CRATES">
						                      		<#assign gTotcrates=typeEntry.getValue()>
						                      		<#assign cratesValue=StringUtil.split(gTotcrates,"-")>
						                      		<#assign totCrates=(totCrates+Static["java.lang.Integer"].valueOf(cratesValue[0]))>
						                      	</#if>
						                      	<#if typeEntry.getKey() != "NOPKTS" && typeEntry.getKey() != "NOCRATES">
													<fo:table-cell >
													<#if typeEntry.getKey() != "CASH" && typeEntry.getKey() != "TOTALAMOUNT" >
														<#if typeEntry.getKey() == "LITRES">
						                            		<fo:block  text-align="right"  font-size="8pt">${typeEntry.getValue()}</fo:block>
						                            		<#else>
						                            			<fo:block  text-align="right"  font-size="8pt">${typeEntry.getValue()}</fo:block>
							...
							<fo:table-row >
	                 				...
	                 				<fo:table-cell>
	                     				<fo:block text-align="left">${totCrates}-0</fo:block>
	                 				</fo:table-cell>
	                  				<fo:table-cell>
	                    	 			<fo:block text-align="center">${gTotLitres?string("##0.0")}*</fo:block>
	                 				</fo:table-cell>	
	            				</fo:table-row>
	            				<fo:table-row>
	            					<fo:table-cell>
