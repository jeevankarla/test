            ...
            while (oit.hasNext()) {
                ...
                while (oait.hasNext()) {
                    ...
                    while (oabit.hasNext()) {
                        ...
                        if (includeInTax || "SALES_TAX".equals(orderAdjustmentTypeId) || "SHIPPING_CHARGES".equals(orderAdjustmentTypeId)) {
                            if (orderItemSeqId.equals(orderItem.get("orderItemSeqId"))) {
...
Debug.logError("startDate=" + startDate + "; endDate=" + endDate, module);	
        ...
        List boothOrdersList = FastList.newInstance();		
		try {
			boothOrdersList = delegator.findList(
					"OrderHeaderFacAndItemBillingInv", paramCond, null,
					UtilMisc.toList("estimatedDeliveryDate"), findOptions,
					false);
			Set invoiceIdSet = new HashSet(EntityUtil
					.getFieldListFromEntityList(boothOrdersList, "invoiceId",
							true));
			...
			for (int i = 0; i < invoiceIdList.size(); i++) {
				...
				Map<String, Object> paymentCtx = UtilMisc
						.<String, Object> toMap("paymentTypeId", paymentType);
				paymentCtx.put("paymentMethodTypeId", paymentMethodType);
				GenericValue facility = delegator.findOne("Invoice", UtilMisc
						.toMap("invoiceId", invoiceId), false);
				...
				Map<String, Object> paymentResult = dispatcher.runSync(
						"createPayment", paymentCtx);
				if (ServiceUtil.isError(paymentResult)) {
					return ServiceUtil.returnError(null, null, null,
							paymentResult);
				}
				String paymentId = (String) paymentResult.get("paymentId");
				Map<String, Object> invoiceCtx = UtilMisc
						.<String, Object> toMap("invoiceId", invoiceId);
				invoiceCtx.put("userLogin", userLogin);
				invoiceCtx.put("statusId", "INVOICE_READY");
				try {
					Map<String, Object> invoiceResult = dispatcher.runSync(
							"setInvoiceStatus", invoiceCtx);
				...
				Map<String, Object> invoiceApplCtx = UtilMisc
						.<String, Object> toMap("invoiceId", invoiceId);
				...
				Map<String, Object> invoiceApplResult = dispatcher.runSync(
						"createPaymentApplication", invoiceApplCtx);
