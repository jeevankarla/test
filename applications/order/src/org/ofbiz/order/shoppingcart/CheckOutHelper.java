...
public class CheckOutHelper {
    ...
    protected boolean simpleLMSOrder = false;
    ...
    public CheckOutHelper(LocalDispatcher dispatcher, Delegator delegator, ShoppingCart cart, boolean simpleLMSOrder) {
        this.delegator = delegator;
        this.dispatcher = dispatcher;
        this.cart = cart;
        this.simpleLMSOrder = simpleLMSOrder;
    }
    ...
    public Map<String, Object> createOrder(GenericValue userLogin, String distributorId, String affiliateId,
        ...
        if (this.simpleLMSOrder) {
        	context = this.cart.makeCartMapSimpleLMSOrder(this.dispatcher, areOrderItemsExploded);
        } else {
        	context = this.cart.makeCartMap(this.dispatcher, areOrderItemsExploded);
        }
...
*/
    ...
    public void calcAndAddTax(List<GenericValue> taxItemTypes) throws GeneralException {
        calcAndAddTax(null, false ,taxItemTypes);
    }
    
    public void calcAndAddTax(GenericValue shipAddress, boolean skipEmptyAddresses ,List<GenericValue> taxItemTypes) throws GeneralException {
    ...
    //Editing old order same copied from createOrder
    public Map<String, Object> editOrder(GenericValue userLogin) {
        return editOrder(userLogin, null, null, null, false, null, cart.getWebSiteId());
    }

    // Create order event - uses createOrder service for processing
    public Map<String, Object> editOrder(GenericValue userLogin, String distributorId, String affiliateId,
            List<GenericValue> trackingCodeOrders, boolean areOrderItemsExploded, String visitId, String webSiteId) {
        if (this.cart == null) {
            return null;
        }
        ...
        try {
            storeResult = dispatcher.runSync("editOrder", context);
            orderId = (String) storeResult.get("orderId");
            if (UtilValidate.isNotEmpty(orderId)) {
                this.cart.setOrderId(orderId);
                if (this.cart.getFirstAttemptOrderId() == null) {
