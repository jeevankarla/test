...
public class TaxAuthorityServices {
     ...
     */    
    public static List<List<GenericValue>> consolidateItemAdjustments(List<List<GenericValue>> itemAdjustments) {
    	Map<String, GenericValue> consolidatedItemMap = FastMap.newInstance();
    	List<List<GenericValue>> resultList = FastList.newInstance();
        for (int i = 0; i < itemAdjustments.size(); i++) {
        	List<GenericValue> adjList = itemAdjustments.get(i);
        	for (int j = 0; j < adjList.size(); j++) {
        		String adjustmentType = adjList.get(j).getString("orderAdjustmentTypeId");
        		GenericValue consolidatedItem = consolidatedItemMap.get(adjustmentType);
        		if (consolidatedItem == null) {
        			consolidatedItemMap.put(adjustmentType, adjList.get(j));
        		}
        		else {
        			BigDecimal amount = consolidatedItem.getBigDecimal("amount");
        			amount = amount.add(adjList.get(j).getBigDecimal("amount"));
        			consolidatedItem.put("amount", amount);
        		}
        	}
        }
        for (int i = 0; i < itemAdjustments.size() - 1; i++) {
        	List<GenericValue> elem = FastList.newInstance();
        	resultList.add(elem);
        }
        List<GenericValue> consolidatedAdjList = FastList.newInstance();
        Iterator entries = consolidatedItemMap.entrySet().iterator();
        while (entries.hasNext()) {
          Entry<String, GenericValue> entry = (Entry) entries.next();
          consolidatedAdjList.add(entry.getValue());
        }
        resultList.add(consolidatedAdjList);
        return resultList;
    }
    ...
    private static List<GenericValue> getTaxAdjustmentsOrig(Delegator delegator, GenericValue product, GenericValue productStore, 
    ...
    private static List<GenericValue> getTaxAdjustments(Delegator delegator, GenericValue product, GenericValue productStore, 
            ...
            BigDecimal itemPrice, BigDecimal itemQuantity, BigDecimal itemAmount, 
            BigDecimal shippingAmount, BigDecimal orderPromotionsAmount) {
    	return getTaxAdjustments(delegator, product, productStore,payToPartyId,billToPartyId,taxAuthoritySet, itemPrice,itemQuantity, itemAmount, 
                shippingAmount ,orderPromotionsAmount ,null,null);
    }
    ...
    private static List<GenericValue> getTaxAdjustments(Delegator delegator, GenericValue product, GenericValue productStore, 
    	...
    	return getTaxAdjustments(delegator, product, productStore,payToPartyId,billToPartyId,taxAuthoritySet, itemPrice,itemQuantity, itemAmount, 
                shippingAmount ,orderPromotionsAmount ,null ,taxTypeList);
    }
    
    public static List<GenericValue> getTaxAdjustmentByType(Delegator delegator, GenericValue product, GenericValue productStore, 
            BigDecimal itemPrice, BigDecimal itemQuantity, BigDecimal itemAmount, 
            BigDecimal shippingAmount,String taxTypeId, List<GenericValue> taxTypeList) {
    	return getTaxAdjustments(delegator, product, productStore, null,null, null, itemPrice,itemQuantity, itemAmount, 
                shippingAmount ,null ,taxTypeId ,taxTypeList);
    }
    
    private static List<GenericValue> getTaxAdjustments(Delegator delegator, GenericValue product, GenericValue productStore, 
            String payToPartyId, String billToPartyId, Set<GenericValue> taxAuthoritySet, 
            BigDecimal itemPrice, BigDecimal itemQuantity, BigDecimal itemAmount, 
            BigDecimal shippingAmount, BigDecimal orderPromotionsAmount , String taxTypeId ,List<GenericValue> taxTypeList) {
        Timestamp nowTimestamp = UtilDateTime.nowTimestamp();
        List<GenericValue> adjustments = FastList.newInstance();
