...
<script type="application/javascript">
	...
	function createdisbursement(form) {
		...
		message = "";
		message += "<form action='createEmpAdvDisbursement' method='post' onsubmit='return disableSubmitButton();'>";
			message += "<table cellspacing=10 cellpadding=10 border=2 width='160%'>";
		$('#parameters tr').each(function(i, row){
			var tdObj = $(row).find('td');
			message += "<tr>";
			$(tdObj).each(function(j, cell){
				var custRequestId = $($(cell).find("#custRequestId")).val();
				var amount = $($(cell).find("#amount")).val();
				var partyId = $($(cell).find("#partyId")).val();
				message += "<tr class='h2'><td align='left'class='h3' width='60%'>CustRequestId: <input type=hidden name='custRequestId_o_"+i+"' value='"+custRequestId+"'>"+custRequestId+"</td><td align='left'class='h3' width='60%'>PartyId: <input type=hidden name='partyId_o_"+i+"' value='"+partyId+"'>"+partyId+"</td>";
		   ...
		   message += "<tr class='h2'><td align='left' class='h3' width='60%'>Cheque Date:</td><td align='left' width='60%'><input class='h4' type='text' readonly id='instrumentDate' name='instrumentDate' onmouseover='datepick()'/></td></tr>" +
			...
			message += "</tr>";	
		title = "<center>Disbursement <center><br />";
		message += "</table></form>";
		Alert(message, title);
	};
</script>
