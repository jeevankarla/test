        ...
        if(checkedInvoices.size() > 0) {
        	...
        	jQuery.each(checkedInvoices, function() {
            	if (jQuery(this).is(':checked')) {
					...
					var invoiceId=$($(domObj).find("#invId")).val();
...
//]]>
	...
	function massPaymentSubmit(current){
	 ...
	 $("#listInvoices").append(invoiceList);
</script>
  ...
  <form name="listInvoices" id="listInvoices"  method="post" action="">
    <div align="right">
      <select name="serviceName" id="serviceName" onchange="javascript:setServiceName(this);">
        ...
        <option value="<@ofbizUrl>acctgTransDetailReportPdf1.pdf</@ofbizUrl>">Accounting Report</option>
          ...
          <#assign invoicePaymentInfo = invoicePaymentInfoList.get("invoicePaymentInfoList").get(0)?if_exists>
           <#assign invoiceSequence = Static["org.ofbiz.accounting.invoice.InvoiceServices"].getInvoiceSequence(delegator, invoice.invoiceId)?if_exists/>
            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
              <td><a class="buttontext" href="<@ofbizUrl>APInvoiceOverview?invoiceId=${invoice.invoiceId?if_exists}&amp;subTabButtonValue=${tabButtonItem5?if_exists}&amp;invoiceTypeId=${invoice.invoiceTypeId?if_exists}&amp;finAccountId=${parameters.finAccountId?if_exists}&amp;finAccountTransTypeId='GRANT_DISB'</@ofbizUrl>">${invoice.get("invoiceId")?if_exists}</a></td>
              <td>
