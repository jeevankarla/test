...
//<![CDATA[
    ...
    function toggleInvoiceId(master) {
        var invoices = jQuery("#listInvoices :checkbox[name='paymentIds']");
    ...
    function getInvoiceRunningTotal() {
		var checkedInvoices = jQuery("input[name='paymentIds']:checked");
		var index = 0;
    ...
    function processPayments(current){
    	...
    	var invoices = $('input[name=paymentIds]:checked');
    	 if(invoices.size() <=0) {
        ...
        jQuery.each(invoices, function() {
            if (jQuery(this).is(':checked')) {
            	...
            	var invoicepaymentObj = $(domObj).find("[name='invoicepaymentId']");
            	...
            	var invoicepaymentId = $(invoicepaymentObj).val();
            	var amount = parseFloat($(eachAmtObj).val());
            	var partyId = $(partyIdObj).val();
            	var fromPartyId = $(fromPartyIdObj).val();
            	
               	var appendStr = "<input type=hidden name=paymentId_o_"+index+" value="+invoicepaymentId+" />";
    	...
    	appStr += "<input type=hidden name=payAmount value='"+ payAmount +"' />";
...
<#if paymentLis?has_content>
<form name="groupInvoiceForm" id="groupInvoiceForm" method="post" action="makeSingleGroupInvoicePayments">
</form>


  <div>
    <span class="label">Total Payments:${maximumSize?if_exists}</span>  
    <span class="label">${uiLabelMap.AccountingRunningTotalOutstanding} (Selected Payments) :</span>
  ...
  <form name="listInvoices" id="listInvoices"  method="post" action="">
    ...
    <table class="basic-table hover-bar" cellspacing="0">
      ...
      <tbody>
        ...
        <#list paymentLis as invoice>
         
          
            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
              <td><a class="buttontext" href="<@ofbizUrl>paymentOverview?paymentId=${invoice.paymentId}</@ofbizUrl>">${invoice.get("paymentId")}</a></td>
              
              <input type = "hidden" name = "invoicepaymentId" id = "invoicepaymentId" value = "${invoice.paymentId}">
              
              <td>${(invoice.paymentDate)?if_exists}</td>              
              
              <td>${(invoice.description)?if_exists}</td>
              <#assign partyName= Static["org.ofbiz.party.party.PartyHelper"].getPartyName(delegator, invoice.partyIdTo, false)?if_exists/>
            
            
              <input type = "hidden" name = "partyId" id = "partyId" value = "${invoice.partyIdFrom}">
              <input type = "hidden" name = "fromPartyId" id = "fromPartyId" value = "${invoice.partyIdTo}">
        
           
              <input type = "hidden" name = "partyIdName" id = "partyIdName" value = "${partyName}">
              <input type = "hidden" name = "voucherTypeId" id = "voucherTypeId" value = "${invoice.paymentMethodTypeId?if_exists}">
              ...
              <td><a href="/partymgr/control/viewprofile?partyId=${invoice.partyIdTo}">${Static["org.ofbiz.party.party.PartyHelper"].getPartyName(delegator, invoice.partyIdTo, false)?if_exists} [${(invoice.partyIdTo)?if_exists}]</a></td>
              <td align="right"><@ofbizCurrency amount=invoice.amount isoCode=defaultOrganizationPartyCurrencyUomId />
              	<input  type="hidden" name="tempAmount" id="tempAmount" value="${invoice.amount?if_exists?string("#0.00")}" / >
              </td>	
              
              
             <td align="right"><input type="checkbox" id="invoiceId_${invoice_index}" name="paymentIds" value="${invoice.paymentId}" onclick="javascript:getInvoiceRunningTotal();"/></td>
            </tr>
