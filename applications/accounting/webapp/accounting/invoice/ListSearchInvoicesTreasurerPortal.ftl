        ...
        <#list invoiceList as invoice>
          ...
          <#assign invoicePaymentInfo = invoicePaymentInfoList.get("invoicePaymentInfoList").get(0)?if_exists>
            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
              <td><a class="buttontext" href="<@ofbizUrl>APInvoiceOverview?invoiceId=${invoice.invoiceId?if_exists}&amp;subTabButtonValue=${tabButtonItem5?if_exists}&amp;finAccountId=${parameters.finAccountId?if_exists}&amp;finAccountTransTypeId='GRANT_DISB'</@ofbizUrl>">${invoice.get("invoiceId")?if_exists}</a></td>
              <td>
              ...
              <#if invoice.shipmentId?has_content>
 		      ...
 		      <td>${(shipment.supplierInvoiceId)?if_exists}</td>
 		      <#else>
 		      	<td></td>
			 </#if>
