...
if(UtilValidate.isNotEmpty(parameters.pmntVoidRecrdsFlag) && "Y".equals(parameters.pmntVoidRecrdsFlag)){
	conditionList.add(EntityCondition.makeCondition(EntityCondition.makeCondition("statusId", EntityOperator.NOT_EQUAL, "PMNT_VOID"),EntityOperator.OR,
					EntityCondition.makeCondition([
						EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "PMNT_VOID"),
						EntityCondition.makeCondition("cancelDate", EntityOperator.NOT_EQUAL, null),
						EntityCondition.makeCondition("chequeReturns", EntityOperator.EQUALS, "Y"),
						],EntityOperator.AND)))
}else{
...
if(UtilValidate.isNotEmpty(parameters.pmntVoidRecrdsFlag) && "Y".equals(parameters.pmntVoidRecrdsFlag)){
	...
	if(UtilValidate.isNotEmpty(partyfromDate)&& UtilValidate.isNotEmpty(partythruDate)){
		conditionList.add(EntityCondition.makeCondition("cancelDate", EntityOperator.GREATER_THAN_EQUAL_TO,UtilDateTime.getDayStart(fromDateTime)))
		conditionList.add(EntityCondition.makeCondition("cancelDate",EntityOperator.LESS_THAN_EQUAL_TO, UtilDateTime.getDayEnd(thruDateTime)))
		}
	if(UtilValidate.isNotEmpty(fromDate)&& UtilValidate.isNotEmpty(thruDate)){
	conditionList.add(EntityCondition.makeCondition("cancelDate", EntityOperator.GREATER_THAN_EQUAL_TO,UtilDateTime.getDayStart(fromDateTime)))
	conditionList.add(EntityCondition.makeCondition("cancelDate",EntityOperator.LESS_THAN_EQUAL_TO, UtilDateTime.getDayEnd(thruDateTime)))
...
if(UtilValidate.isNotEmpty(segmentId) && segmentId.equals("YARN_SALE"))
	...
	while (payment = payBounceIterator.next()) {
		...
		payCounter=payCounter+1;
		Map pmtVoidDataMap=[:];
		curntDay=UtilDateTime.toDateString(payment.cancelDate ,"dd-MM-yyyy");
		
		pmtVoidDataMap["date"]=curntDay;
		pmtVoidDataMap["paymentDate"]=payment.paymentDate;
		pmtVoidDataMap["paymentId"]=payment.paymentId;
		pmtVoidDataMap["paymentMethodTypeId"]=payment.paymentMethodTypeId;
		pmtVoidDataMap["description"]=payment.comments;
		pmtVoidDataMap["amount"]=payment.amount;
		pmtVoidDataMap["crOrDbId"]="";
		pmtVoidDataMap["debitValue"]=0;
		pmtVoidDataMap["creditValue"]=0;
		
		pmtVoidDataMap["partyId"]=payment.partyIdTo;
		pmtVoidDataMap["vchrType"]="DISBURSEMENT";
		pmtVoidDataMap["crOrDbId"]="D";
		pmtVoidDataMap["vchrCode"]="Payment";
		pmtVoidDataMap["debitValue"]=payment.amount;
		pmtVoidDataMap["crpmtVoidDataMape"]=0;
		if(UtilValidate.isEmpty(payment.comments)){
			pmtVoidDataMap["description"]="Payment Amount";
		}
		//preparing Map here
		dayPaymentList=[];
		dayPaymentList=apPaymentDetailsMap[curntDay];
		...
		if(UtilValidate.isEmpty(dayPaymentList)){
			dayTempPaymentList=[];
			dayTempPaymentList.addAll(pmtVoidDataMap);
			dayPaymentList=dayTempPaymentList;
			apPaymentDetailsMap.put(curntDay, dayPaymentList);
		}else{
			 dayPaymentList.addAll(pmtVoidDataMap);
			apPaymentDetailsMap.put(curntDay, dayPaymentList);
		}
		apPaymentDetailsMap.put("amount",apPaymentDetailsMap.get("amount").add(payment.amount));
