...
if ("Y".equals(parameters.noConditionFind)) {
    ...
    if (fromDate) {
        paymentCond.add(EntityCondition.makeCondition("effectiveDate", EntityOperator.GREATER_THAN_EQUAL_TO, UtilDateTime.getDayStart(fromDate)));
    }
    if (thruDate) {
        paymentCond.add(EntityCondition.makeCondition("effectiveDate", EntityOperator.LESS_THAN_EQUAL_TO, UtilDateTime.getDayEnd(thruDate)));
    }
    if (partyIdFrom) {
    ...
    if (finAccountId) {
        ...
        if (finAccountTransList) {
            ...
            payments = delegator.findList("PaymentAndType", EntityCondition.makeCondition(paymentCond, EntityOperator.AND), null, null, null, false);
        }
    } else {
