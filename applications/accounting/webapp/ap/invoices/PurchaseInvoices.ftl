...
function setServiceName(selection) {
    ...
    if ( selection.value == 'bulkSms') {
        document.listPurchaseInvoices.action = jQuery('#bulkSms').value;
    }
    else {
...
<#if invoices?has_content >
  ...
  <form name="listPurchaseInvoices" id="listPurchaseInvoices"  method="post" action="javascript:void();">
    ...
    <input type="hidden" name="bulkEmail" id="bulkSms" value="<@ofbizUrl>bulkEmail</@ofbizUrl>"/>
