...
public class ProductionRunServices {
    ...
    public static Map<String, Object> createProductionRun(DispatchContext ctx, Map<String, ? extends Object> context) {
        ...
        while (rt.hasNext()) {
            ...
            if (EntityUtil.isValueActive(routingTaskAssoc, startDate)) {
                ...
                cloneWorkEffortAttributes(ctx, userLogin, routingTaskId, productionRunTaskId);
    ...
    /**
     * Make a copy of the attributes that were defined on the template routing task to the new production run task.
     */
    private static void cloneWorkEffortAttributes(DispatchContext dctx, GenericValue userLogin, String routingTaskId, String productionRunTaskId) {
        List<GenericValue> workEffortAttributes = null;
        try {
            workEffortAttributes = dctx.getDelegator().findByAnd("WorkEffortAttribute", UtilMisc.toMap("workEffortId", routingTaskId));
        } catch (GenericEntityException e) {
            Debug.logError(e.getMessage(),  module);
        }

        if (workEffortAttributes != null) {
            for (GenericValue attribute : workEffortAttributes) {
                Map<String, Object> createAttribute = UtilMisc.toMap(
                        "workEffortId", productionRunTaskId,
                        "attrName", attribute.getString("attrName"),
                        "attrValue", attribute.getString("attrValue"), "userLogin", userLogin);

                try {
                    dctx.getDispatcher().runSync("createTaskAttribute", createAttribute);
                } catch (GenericServiceException gse) {
                    Debug.logError(gse, "Problem calling the createTaskAttribute service", module);
                }
            }
        }
    }    
