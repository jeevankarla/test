...
if(UtilValidate.isNotEmpty(employeeList)){
	employeeList.each{ employee->
		...
		if(UtilValidate.isNotEmpty(EmplJoiningDateList)){
			...
			nowDate = UtilDateTime.getDayEnd(UtilDateTime.nowTimestamp());
			if(UtilValidate.isNotEmpty(EmplappointmentDate)){
				totalDays= UtilDateTime.getIntervalInDays(EmplappointmentDate,nowDate);
			}
		}
