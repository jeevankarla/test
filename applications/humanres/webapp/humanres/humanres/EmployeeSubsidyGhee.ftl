  ...
  return (x == y ? 0 : (x > y ? 1 : -1));
	...
	function setUpItemList() {
			...
			var columns = [		
					...
					{id:"boothId", name:"Booth Id", field:"boothId", width:90, minWidth:100, cssClass:"cell-title", editor:FloatCellEditor},
					<#if productList?has_content> 
				        <#list productList as prod>	
								{id:"${prod.productId?if_exists}", name:"${prod.brandName?if_exists}", field:"${prod.productId?if_exists}", width:100, minWidth:80, editor:FloatCellEditor}<#if prod_has_next>,</#if>
						</#list>
					</#if>
			];
...
function editClickHandler(row) {
		updateEmployeeMilkCardsInternal('updateEmployeeSubsidyGhee', '<@ofbizUrl>updateEmployeeSubsidyGhee</@ofbizUrl>', row);
	}
function updateEmployeeMilkCardsInternal(formName, action, row) {
	...
	}//end of updateEmployeeMilkCardChargesInternal
function selectDeptForMilkCards(){
	var partyIdTo=jQuery("#FindEmployeeMilkCards_partyIdTo").val();
	...
	$.ajax({
        ...
        success: function(result) {
       	...
       	$("#FindEmployeeMilkCards_partyId").val(partyId);
       	 },
       error: function() {
...
<div id="wrapper" style="width: 95%; height:100%">
  <form method="post" name="updateEmployeeSubsidyGhee" id="updateEmployeeSubsidyGhee"> 
 </form>
</div>
...
<div id="div2" style="float: left;width: 100%;align:right; border: #F97103 solid 0.1em;">
    ...
    <div>    	
 		<div class="grid-header" style="width:100%">
			<label>Employee Subsidy Ghee: <#if timePeriodStart?has_content>[${(Static["org.ofbiz.base.util.UtilDateTime"].toDateString(timePeriodStart?if_exists, "MMMMM-yyyy"))}]</#if></label>
		</div>    
