				...
				if (args.cell == 2) {
			  	...
			  	if (args.cell == 2) {
					...
					if(isNaN(eligibilityAmountValue)){
			  		...
			  		if( actualAmountValue > eligibilityAmountValue){
			  			var amount = actualAmountValue-eligibilityAmountValue;
			  			data[args.row]["excessAmount"] = amount;
			  			grid.updateRow(args.row);
			  		}else{
			  			data[args.row]["excessAmount"] = 0;
			  			grid.updateRow(args.row);
			  		}
			  		//jQuery("#excessAmount").html(amount);
			  	}
...
function editClickHandler(row) {
		updateTelephoneCUGChargesInternal('updateTelephoneCUGCharges', '<@ofbizUrl>updateTelephoneCUGCharges</@ofbizUrl>', row);
	}
function updateTelephoneCUGChargesInternal(formName, action, row) {
	...
	}//end of updateTelephoneCUGChargesInternal
...
<div id="wrapper" style="width: 95%; height:100%">
  <form method="post" name="updateTelephoneCUGCharges" id="updateTelephoneCUGCharges"> 
 </form>
</div>
...
<div id="div2" style="float: left;width: 100%;align:right; border: #F97103 solid 0.1em;">
    ...
    <div>    	
 		<div class="grid-header" style="width:100%">
			<label>Telephone CUG Charges: <#if timePeriodStart?has_content>[${(Static["org.ofbiz.base.util.UtilDateTime"].toDateString(timePeriodStart?if_exists, "MMMMM-yyyy"))}]</#if></label>
		</div>    
