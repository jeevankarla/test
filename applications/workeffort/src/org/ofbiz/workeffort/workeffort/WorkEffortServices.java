...
public class WorkEffortServices {
    ...
    private static List<EntityCondition> getDefaultWorkEffortExprList(String calendarType, Collection<String> partyIds, String workEffortTypeId, List<EntityCondition> cancelledCheckAndList) {
        ...
        if ("CAL_PERSONAL".equals(calendarType)) {
            ...
            List<EntityCondition> publicEvents = UtilMisc.<EntityCondition>toList(
                    EntityCondition.makeCondition("scopeEnumId", EntityOperator.EQUALS, "WES_HR")      
                    );           
    ...
    `    3) in the next screen, click on the "Confirm" link (top part of the sccreen)
        ...
        String localDispatcherName=((dispatcher.getName().split("#"))[0]);        
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        ...
        List<GenericValue> emplRecord=null;
        try{
             ...
             emplRecord=delegator.findList("Employment",condition , null, null, null, false);  
        ...
        if ( UtilValidate.isNotEmpty(scopeEnumId) && scopeEnumId == "WES_HR" && (emplRecord.size() >0) ) {
            entityExprList.add(EntityCondition.makeCondition("scopeEnumId", EntityOperator.EQUALS, scopeEnumId));           
        }       
        if((emplRecord.isEmpty()) && scopeEnumId == "WES_HR" ){
        	 entityExprList.add(EntityCondition.makeCondition("scopeEnumId", EntityOperator.EQUALS, ""));        	 
        }
        if((emplRecord.isEmpty()) && scopeEnumId != "WES_HR" ){
       	 entityExprList.add(EntityCondition.makeCondition("scopeEnumId", EntityOperator.NOT_EQUAL, "WES_HR"));        	 
       }
    ...
    protected static void processEventReminder(DispatchContext ctx, GenericValue reminder, Map<String, Object> parameters) throws GenericEntityException {
        ...
        if (contactMech != null && "TELECOM_NUMBER".equals(contactMech.get("contactMechTypeId"))) {
        	GenericValue telecomNumber = contactMech.getRelatedOne("TelecomNumber");
        	if(telecomNumber == null || telecomNumber.isEmpty()){
        		return;
        	}
        	GenericValue workEffort = reminder.getRelatedOne("WorkEffort"); 
        	String contactNumberTo=telecomNumber.getString("contactNumber");
        	try {
        		//Date
        		Date eventDate = new Date();
        		eventDate = (Date)workEffort.get("estimatedStartDate");
                SimpleDateFormat sd = new SimpleDateFormat("dd/MM/yyyy HH:mm aa");
                String estimatedStartDate=sd.format(eventDate);
                String workEffortName=(String)workEffort.get("workEffortName");                
                String text="REMINDER ["+estimatedStartDate+" Meeting]:"+workEffortName;
                String partyId = (String)reminder.get("partyId");
                String partyIdFrom="system";
                Map smsCtx = UtilMisc.toMap("contactNumberTo", contactNumberTo, "text", text, "partyId", partyId, "partyIdFrom", partyIdFrom);         
            	dispatcher.runAsync("sendSms", smsCtx);
            } catch (Exception e) {
                Debug.logWarning("Error while Sms event reminder - workEffortId = " + reminder.get("workEffortId") + ", contactMechId = " + reminder.get("contactMechId") + ": " + e, module);
            }
            return;
        }
