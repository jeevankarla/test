...
if(start > fromDate && start<thruDate){
    start = UtilDateTime.getWeekStart(start, timeZone, locale);
     }
    else{  
    request.setAttribute("_ERROR_MESSAGE_", "not allowed to set!");
...
//Debug.logInfo("in next week==== " +  next,"");
if(next > thruDate ){
context.put("nextFlag","false");
//Debug.logInfo("in next flag==== " +  next,"");
}
else{
context.put("nextFlag","true");
}
...
Map serviceCtx = UtilMisc.toMap("userLogin", userLogin,"start",start,"numPeriods", 7,
        "periodType", Calendar.DATE,"scopeEnumId",scopeEnumId);
serviceCtx.putAll(UtilMisc.toMap("partyId", partyId, "facilityId", facilityId, 
        "fixedAssetId", fixedAssetId, "workEffortTypeId", workEffortTypeId, "calendarType", calendarType,"scopeEnumId",scopeEnumId,
