...
if(shipListType.equals("overrideList")){
	absenteeOverrideMap = NetworkServices.getAbsenteeOverrideBooths(dctx , [overrideSupplyDate:supplyDate]);
	context.absenteeOverrideList = absenteeOverrideMap.getAt("overrideList");
	
}else {
	if(hideSearch == "N"){
		stopShipMap = NetworkServices.getStopShipList(dctx , [supplyDate:supplyDate , facilityId : facilityId ,userLogin : userLogin]);
