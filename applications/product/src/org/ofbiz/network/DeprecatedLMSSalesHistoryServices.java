...
@Deprecated
public class DeprecatedLMSSalesHistoryServices {
	
	public static final String module = DeprecatedLMSSalesHistoryServices.class.getName();
	    ...
	    public static Map<String, Object>  populateLMSSalesHistorySummary(DispatchContext dctx, Map<String, Object> context)  {
			...
			try {
				...
				if (salesSummary == null) {
					...
					salesSummary.create();  
					DeprecatedLMSSalesHistoryServices.LMSSalesHistorySummaryDetail(dctx,  UtilMisc.toMap("salesDate", summaryDate));
				}
				else {
			...
			try{
				for(int i=0;i<=IntervalDays;i++){
					...
					if(monthEnd.equals(currentDate)){
						...
						DeprecatedLMSSalesHistoryServices.populateLMSMonthlySalesSummary(dctx, context, "SALES_MONTH",monthStart,monthEnd);
					}
				}//days
			} catch (GenericEntityException e) {
				Debug.logError(e, module);
        		...
        		for(Object shipment : shipmentList){
    		    	...
    		    	while(shipTypeIter.hasNext()) {
    			    	...
    			    	while(prodSubTypeIter.hasNext()) {
    				    	...
    				    	totQty = DeprecatedLMSSalesHistoryServices.sum(qtyForPeriod);
    				    	totRevenu = DeprecatedLMSSalesHistoryServices.sum(revenuForPeriod);
